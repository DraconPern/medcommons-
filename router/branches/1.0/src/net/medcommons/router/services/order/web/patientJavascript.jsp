...
<c:set var="ccrDOM" value="${ccr.JDOMDocument}" scope="page"/>
  var newObj;
  var newSeries;
  p = new Patient('${mc:jsEscape(ccr.patientGivenName)} ${mc:jsEscape(ccr.patientFamilyName)}',"<mc:xvalue bean='ccrDOM' path='patientMedCommonsId'/>");
  p.studies[0] =  new Study(
      "Study",
    "${ccr.guid}", 
    "<dt:format pattern="MM/dd/yyyy">${ccr.createTimeMs}</dt:format>",
    "<dt:format pattern="MM/dd/yyyy">${ccr.createTimeMs}</dt:format>");
  p.studies[0].pending = ${! ccr.validated };
