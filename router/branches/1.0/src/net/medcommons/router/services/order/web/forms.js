...
function emailShareForm(options) {
	...
	var form =FORM({action: 'SharePHR.action'}, input('hidden','fromAccount',options.patientId), table( 
        ...
        formRow('To', text('toEmail',options.to)),
        formRow('Subject', text('subject',options.subject)), 
	            ...
	            if(result.status == 'ok') { 
	                ...
	                if(form.control && (params.control=='pin')) {
	                    appendChildNodes('waitMessage',  
	                            DIV({style:'margin-left: 2em; maring-top: 0.5em;'},'Tracking Number: ',prettyTrack(result.trackingNumber),' PIN: ', result.pin));
	                }
