/**
 * $Id: StartDDLAction.java 3903 2010-10-28 06:27:35Z ssadedin $
 */
...
import net.medcommons.rest.JSONResult;
...
public class StartDDLAction extends BaseActionBean implements ValidationErrorHandler {
    ...
    public Resolution startDDL(){
        try {
            ...
            ctx.getResponse().addHeader("Content-Disposition", "Inline; fileName=start.jnlp");
    ...
    /**
     * Hash of commands waiting for DDLs keyed on DDLID
     */
   private static Map<String, List<JSONObject>> ddls = new HashMap<String, List<JSONObject>>();
    ...
    */
   @JSON
   public Resolution command() {
        
        // Because caching is disabled by default for this action, we have to do it here manually
        nocache();
        
       log.info("Received command for ddl " + ddlid);
       
       synchronized(ddls) {
           List<JSONObject> cmds = ddls.get(ddlid);
           if(cmds==null) {
               ddls.put(ddlid, cmds=new ArrayList<JSONObject>());       
           }
