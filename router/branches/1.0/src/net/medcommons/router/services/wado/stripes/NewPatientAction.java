/*
 * $Id: NewPatientAction.java 3532 2009-11-02 03:17:09Z ssadedin $
    ...
    public Resolution create() {
        ...
        try {
            ...
            if(!blank(this.oauth_consumer_key)) {
                ...
                share.setIdentityType("Application");
            ...
            return new StreamingResolution("text/plain", String.format("{status:'ok',patientMedCommonsId:'%s',currentCCRGuid:'%s',auth:'%s',secret:'%s'}",
                            patientId, guid, authToken, secret));
        }
        catch (Exception e) {
            log.error("Failed to create new patient " + this.givenName + " " + this.familyName,e);
            return new StreamingResolution("text/plain", "{status:'failed',error:'"
                            + Str.escapeForJavaScript(e.getMessage()) + "'}");
        }
    }
