 ...
 *
 *    4.  Autocompletion will happen automatically when you type on the field. But if you
 *        want to invoke it manually, use 
 *
 *          field.beginAutoComplete();
 *
 *  Advanced: you can control the behavior by setting autocompleteBehavior.
 *  One way to do this is as part of the init_autocomplete() call.  If you do this
 *  it is important to ensure auto_complete_reset_behavior() is called when
 *  the autocompletion ends.
 */
...
/********************************************************/     
     ...
     */
    function init_autocomplete(field, completions, bhv) {
      field.values = completions;
      field.onkeyup=autocomplete;
      field.onkeydown=auto_complete_key_down;
      field.beginAutoComplete = function(){autocomplete.apply(field,[null,true]);}
      if(field.setAttribute)
        field.setAttribute("autocomplete","off");
      if(bhv) {
        field.autocompleteBehavior = bhv;
      }
    }
    ...
    function autocomplete(evt, forceShow) {
      if(!evt)
        evt = window.event;
      ...
      log("autocomplete invoked: key = " + keyCode + " field = " + field.name);
      ...
      if(autocompleteBehavior && autocompleteBehavior.on_begin_auto_complete) {
        autocompleteBehavior.on_begin_auto_complete(evt);        
      }
      ...
      if((keyCode == 9) && (!forceShow)) { // tab key 
        return;
      }
      else
      if(keyCode == 13) { // enter key
        autocompleteBehavior.fill_value(currentCompletion,true);
        auto_complete_cancel_evt(evt);
        return;
      }
      else
      if(keyCode == 40) { // down arrow 
        ...
        log("down arrow, current completion = " + currentCompletion);
        autocompleteBehavior.select_value(currentCompletion, true);
        return;
      }
      else
      ...
      var expr = "^" + matchText;
      if(!autocompleteBehavior.partial_match_allowed) {
        expr+="$";
      }
      var m = new RegExp(expr,"i");
      var found = false;
    ...
    function auto_complete_fill_value(i,requireMatch) {
      var field = autocompleteField;
      if(field == null)
        return;

        
      if(!requireMatch || ((i>=0) && (field.values[i].indexOf(field.value)>=0))) {
        log("Completing value " + field.values[i] + " with match to field " + field.value + " at " + field.values[i].indexOf(field.value) +
    ...
    function auto_complete_highlight_line(i,fillValue) {
      var v = autocompleteField.values;
      var html = '';
      for(j=0;j<v.length;++j) {
        if(i==j) {
          html+='<div class="achighlight ac"><a href="#" onclick="autocompleteBehavior.fill_value('+j+'); return false;" class="achighlight">'+v[j]+'</a></div>';
        }
        else
          html += '<div><a class="ac" href="#" onclick="javascript:autocompleteBehavior.fill_value('+j+'); return false;">'+v[j] +'</a></div>';
      }
      html+='<div id="acinstr">'+autocompleteBehavior.message+'</div>';
      var acdiv = document.getElementById('acdiv');
      acdiv.innerHTML=html;
      if(v[i] != undefined)  {
        if(fillValue && (autocompleteField.value != v[i])) {
          autocompleteField.value = v[i];
        }
      }
    }
...
/********************************************************/    
    function auto_complete_end() {
      log("Ending auto complete due to blur");
      window.setTimeout(function(){document.getElementById('acdiv').style.display='none';},250);
    }
