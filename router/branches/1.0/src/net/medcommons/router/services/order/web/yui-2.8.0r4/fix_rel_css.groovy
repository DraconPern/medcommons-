 ...
 */
def components = [
  'menu','datatable','paginator','tabview','container','button','autocomplete', 'resize', 'layout'
]
...
for(def c in components) {
    ...
    skinDir.eachFileMatch(~/.*\.css/) { f ->
        ...
        f.text = f.text.replace("yui-skin-sam", "yui-skin-mc").replace("assets/skins/sam/", "assets/skins/mc/")    
    }
}