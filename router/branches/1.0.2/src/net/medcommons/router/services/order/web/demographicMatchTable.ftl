...
<tr class='row0'>
  ...
  <td>
    <#if desktop.accountSettings.currentCcrGuid?exists && desktop.accountSettings.currentCcrGuid != ''>
    <a href='<@mc.config property='RemoteAccessAddress'/>/CurrentCCR.action?a=${desktop.ownerMedCommonsId}&auth=${desktop.authenticationToken}' onclick='return open_target_hurl();'><img src='images/hurl.png'/> <@mc.medcommonsId>${desktop.ownerPrincipal.mcId}</@></a>
    <#else>
       <@mc.medcommonsId>${desktop.ownerPrincipal.mcId}</@> 
    </#if>
  </td>
  <td>&nbsp;</td>
</tr>
...
<tr class='row1'>
  ...
  <td><#if action.targetCCR??>${action.targetCCR.patientGivenName} ${action.targetCCR.patientFamilyName}<#else>${desktop.accountSettings.firstName} ${desktop.accountSettings.lastName}</#if></td>
