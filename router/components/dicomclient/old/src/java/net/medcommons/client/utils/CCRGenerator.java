...
import astmOrgCCR.ContinuityOfCareRecordDocument.ContinuityOfCareRecord.Body;
import astmOrgCCR.ContinuityOfCareRecordDocument.ContinuityOfCareRecord.From;
...
import astmOrgCCR.ContinuityOfCareRecordDocument.ContinuityOfCareRecord.To;
...
public class CCRGenerator {
    ...
    private void addPatientActor(Actors actors, CCRCrossReferences ccrCrossReferences, String storageId, DicomMetadata dicomMetadata){
       ...
       for (int i=0;i<f.size(); i++)
       {
        ...
        if (dicomMetadata.getPatientSex() != null){
            ...
            String sexData;
            if ("M".equals(dicomMetadata.getPatientSex()))
            	sexData = "Male";
            else if ("F".equals(dicomMetadata.getPatientSex()))
            	sexData = "Female";
            else
            	sexData = "Other";
            sex.setText(sexData);
     ...
     * @throws IOException
     */
    public ContinuityOfCareRecordDocument initializeCrossReferences(String storageId, List<DicomMetadata> dicomMetadata, List<DicomTransaction> transactions, Properties referenceProper) throws IOException{
    ...
    */
    ...
    private String internalAddDicomReference(References references, CCRCrossReferences crossReferences, DicomTransaction dicomTransaction,
        ...
        List<DicomPreset> presets = getPresetArray(dicomMetadata);
        if (presets != null){
...
private ObjectAttribute addObjectAttribute(CodedDescriptionType cdt, String attribute, Map<String, String> referenceProperties){
     ...
     * @return
     */
    private List<DicomPreset> getPresetArray(DicomMetadata dicomMetadata){
    	ArrayList<DicomPreset> presets = new ArrayList<DicomPreset>();
    	String delimiters = "\\^";
    	String modality = dicomMetadata.getModality();

    	String windowCenter = dicomMetadata.getWindowCenter();
        String windowWidth = dicomMetadata.getWindowWidth();
        if (log.isDebugEnabled())
        	log.debug("windowCenter = " + windowCenter + ", windowWidth=" + windowWidth);
        if ((windowCenter == null) || (windowWidth == null))
        	return(null);
        String windowCenterWidthExplanation = dicomMetadata.getWindowCenterWidthExplanation();
    	...
    	if (windowCenterTokens.countTokens() > 0){
    		while(windowCenterTokens.hasMoreTokens()){
	    		...
	    		int wCenter = FormatUtils.parseIntegerValue(wc);
	    		int wWidth = FormatUtils.parseIntegerValue(ww);
