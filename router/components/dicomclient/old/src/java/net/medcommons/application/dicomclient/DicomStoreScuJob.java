...
public class DicomStoreScuJob  implements Runnable,Job {
    ...
    public void run(){
        ...
        try{
            ...
            else if (exportMethod.equalsIgnoreCase(Commands.EXPORT_METHOD_FILE))
        	...
        	transaction = TransactionUtils.saveTransaction(transaction);
        	if (!success){
        		StatusDisplayManager.getStatusDisplayManager().setErrorMessage("Error exporting data",
    ...
    public void cancelJob(){
     ...
     private void exportViaFILE(List<File> dicomFiles) throws IOException{
             ...
             if (exportFolder == null){
            	 File exportDirectory = ContextManager.getContextManager().getConfigurations().getExportDirectory();
            	 if (exportDirectory == null){
            		 File baseDir = ContextManager.getContextManager().getConfigurations().getBaseDirectory();
            ...
            if (outputDir == null){
                dirname = transaction.getPatientName();
                outputDir = new File(directory, dirname);
                DirectoryUtils.makeDirectory(outputDir);
            }
        ...
        private void exportViaCSTORE(List<File> dicomFiles) throws IOException{
            ...
            if (transaction.getStatus().equals(DicomOutputTransaction.STATUS_COMPLETE)){
            	...
            	log.info(message);
	            StatusDisplayManager.getStatusDisplayManager().setMessage("DICOM Transfer Complete", message, transaction.getDashboardStatusId());
