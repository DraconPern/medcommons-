...
public class ImportHandler {
	...
	public ImportHandler() {
		...
		Thread t = new Thread(closeSeriesTransaction, "CloseSeriesThread");
		t.start();
	}
	 ...
	 * @return
	 */
	public boolean processSOPInstance(String sopInstanceUid) {
		boolean exists = false;
		Store s = DB.get();
		
		List<DicomMetadata> matches = 
		    s.select(DicomMetadata.class,"sopInstanceUid", sopInstanceUid).all();
	    ...
	    List<DicomTransaction> transactions = 
	        s.select(DicomTransaction.class, 
	                "select * from dicom_transaction where seriesinstanceuid = ? and status = ? limit 10",
	                seriesInstanceUid, DicomTransaction.STATUS_ACTIVE)
	                .all();
	    
	    processTransaction(null, transactions, dicomMetadata, false);
	
		return(exists);
	}
	
	/**
	 * Finds the DicomTransaction for the given meta data and
	 * updates the statistics on it.
	 * @param transferKey 
	 */
	public void cstoreEvent(String transferKey, DicomMetadata dicomMetadata) {
		...
		try {
			...
			List<DicomTransaction> transactions = 
			    db.select(DicomTransaction.class, 
			            "select * from dicom_transaction where seriesinstanceuid = ? and status = ? limit 10",
			            new Object[]{ seriesInstanceUid, DicomTransaction.STATUS_ACTIVE})
			            .all();
	          
			processTransaction(transferKey, transactions, dicomMetadata, true);
		}
		catch(Exception e) {
	 ...
	 * 
	 * @param transferKey          optional key to identify the transfer that is causing
	 *                             this item to be imported.
	 * @param transactions
	 * @param dicomMetadata
	 * @param incrementObjectCount
	 */
	private void processTransaction(String transferKey, List<DicomTransaction> transactions, DicomMetadata dicomMetadata, boolean incrementObjectCount)
	{
	    
	    Store db = DB.get();
	    
	    db.requestLock(DicomTransaction.class, dicomMetadata.getSeriesInstanceUid());
	    try {
			if (transactions.size() == 0) {
				DicomTransaction newTx = new DicomTransaction();
				newTx.setStatus(DicomTransaction.STATUS_ACTIVE);
				newTx.setCxpJob(DicomTransaction.UNITIALIZED_CXPJOB);
				newTx.setTimeStarted(System.currentTimeMillis());
				newTx.setLastModifiedTime(System.currentTimeMillis());
				newTx.setSeriesInstanceUid(dicomMetadata.getSeriesInstanceUid());
				newTx.setStudyInstanceUid(dicomMetadata.getStudyInstanceUid());
				newTx.setSeriesDescription(dicomMetadata.getSeriesDescription());
				newTx.setObjectCount(1);
				newTx.setPatientName(dicomMetadata.getPatientName());
				newTx.setPatientSex(dicomMetadata.getPatientSex());
				newTx.setStudyDescription(dicomMetadata.getStudyDescription());
				newTx.setTransferKey(transferKey);
				if (incrementObjectCount)
					newTx.setTotalBytes(dicomMetadata.getLength());
				
				log.info("Created new transaction object:" + newTx.toString());
				StatusDisplayManager.get().setMessage("New DICOM series",
			...
			if (transactions.size() == 1) {
				...
				for(int i = 0; i < transactions.size(); i++) {
					log.error("Entry " + i + " " + transactions.get(i).getLastModifiedTime());
				}
	 ...
	 * checking if the transaction is in a valid state for
	 * the transaction time to be updated.  If not
	 * in a valid state, returns false.
	 * @param transaction
	 */
