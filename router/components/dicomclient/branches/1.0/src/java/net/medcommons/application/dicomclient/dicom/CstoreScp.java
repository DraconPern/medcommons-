...
public class CstoreScp extends StorageService implements Runnable{
    ...
    private static final int PEEK_LEN = 2* 1024;
            ...
            throws IOException, DicomServiceException
    {
    	...
    	Configurations configs = ContextManager.get().getConfigurations();
        ...
        else
        {
            try
            {
                ...
                boolean haveSopInstance = importHandler.processSOPInstance(iuid);
                if (haveSopInstance){
	                ...
	                importHandler.cstoreEvent(null, dicomMetadata);
	                success = true;
                }
    ...
    private void doFileImport(DICOMImportStatus status, File file) throws IOException,
		...
		try {
			...
			importHandler.cstoreEvent(status.getTransferKey(), dicomMetadata);
			success = true;
		} 
		catch(NoSuchAlgorithmException e){
     ...
     * 
     * @param status   status providing an identifier to apply to this import transaction which 
     *                      is carried through to any created DICOM entities
     *                      so that they can be tied back to this import later on. 
     *                      If null, a unique id will be randomly assigned
     * @param f
     * @throws IOException
     */
    public void importDicomFilesInDirectory(DICOMImportStatus status, ImportFiles importFiles, File f) throws IOException {
        
        if("Cancelled" == status.getStatus())
            return;
        ...
        if (f.isDirectory()) {
            ...
            for (int i = 0; i < fs.length; i++)
            	importDicomFilesInDirectory(status, importFiles, fs[i]);
            return;
        }
        
        DicomObject dcmObj = getDICOMObject(f);
        ...
        boolean haveSopInstance = importHandler.processSOPInstance(sopInstanceUID);
        if (haveSopInstance){
    ...
    public static DicomObject getDICOMObject(File f) {
        return getDICOMObject(f, PEEK_LEN);
    }
    
    public static DicomObject getDICOMObject(File f, int peekLen) {
        DicomObject dcmObj = new BasicDicomObject();
        try {
            DicomInputStream in = new DicomInputStream(f);
            try {
                in.setHandler(new StopTagInputHandler(Tag.PixelData));
                in.readDicomObject(dcmObj, (int)Math.min(peekLen,f.length()));
        ...
        // Maybe we didn't read far enough to get the study instance UID - if so, try again
        if(blank(dcmObj.getString(Tag.StudyInstanceUID)) || blank(dcmObj.getString(Tag.SeriesInstanceUID))) {
            if(peekLen > 0 && peekLen < PEEK_LEN * 16 && peekLen < f.length())
                return dcmObj = getDICOMObject(f, peekLen * 4);            
            else
                return null;
        }
