@echo off
rem  Derived from Alan Burlison's blog: http://blogs.sun.com/alanbur/date/20060628
rem compress and sign a JAR file, remove the original
rem  args are <JAR> <keystore> <password> <alias>
rem pack200 --repack --no-keep-file-order --strip-debug %1
echo %1
...
rem  Don't drop the original jar - the original jar is needed to find the pack version.
rem rm %1
