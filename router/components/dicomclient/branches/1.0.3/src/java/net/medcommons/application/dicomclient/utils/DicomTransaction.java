 ...
 * @author mesozoic
 */
@PersistentClass(table="dicom_transaction", idField="id", autoIncrement=true, deleteFields=true,
		         indexes={
                          @PropertyIndex(unique=false,propertyNames={"seriesInstanceUid"}),
					      @PropertyIndex(unique=false,propertyNames={"studyInstanceUid"}),
					      @PropertyIndex(unique=false,propertyNames={"lastModifiedTime"}),
					      @PropertyIndex(unique=false,propertyNames={"status"}),
					      @PropertyIndex(unique=false,propertyNames={"cxpJob"})
				         }
                )
public class DicomTransaction implements ManagedTransaction{
    ...
    public ContextState resolveContextState() {
        ...
        if(contextStateId != null) {
            ContextState result = db.selectSingle(ContextState.class, "id", contextStateId);
            if(result != null)
                return result;
        }
        
        // Can we resolve a patient from the patient matcher?
        DicomNameParser.DicomName name = DicomNameParser.parse(patientName);
        log.info("dicom name = '" + patientName + "', family name is '" + name.family + "'");
        
        ContextState result = PatientMatch.resolveContextState(name.given, name.middle, name.family, null, 
                ContextManager.get().getCurrentContextState());
        ...
        if (storageId.equals(UploadHandler.UNKNOWN_MEDCOMMONS_ACCOUNT)) {
            ...
            // ssadedin: seriously???? selecting ALL the CCRs in the db just to look at the first???
            ResultsIterator<CCRRef> i = db.select(CCRRef.class);
            if (i.hasNext()) {
                CCRRef potentialMatch = i.next();
