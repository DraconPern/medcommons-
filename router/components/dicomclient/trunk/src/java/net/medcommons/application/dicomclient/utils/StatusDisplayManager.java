...
public class StatusDisplayManager {
    ...
    public static TrayImages trayImages = null;
	...
	public StatusDisplayManager() {
		...
		if(ContextUtils.isJDK6Orlater() && SystemTray.isSupported()) {
			...
			if (traySupported){
			    ...
			    trayImages = new TrayImages(startupTooltip, popup);
			    trayIcon = trayImages.trayIcon;
	...
	private static Thread imageStateThread = null;

	public static void setActiveIcon() {
		if(!traySupported) 
		    return;
		
		setImage(trayImages.activeImage);
		
		imageStateThread = new Thread() {
		    int state = 0;
            @Override
            public void run() {
                while(imageStateThread == this) {
                   try { Thread.sleep(1000); } catch (InterruptedException e) { e.printStackTrace(); } 
                   synchronized(trayIcon) {
                       if(imageStateThread == this)
                           StatusDisplayManager.setImage(trayImages.activeImages[state%3]);
                       if(++state>5)
                           state = 0;
                   }
                }
		...
		imageStateThread.setDaemon(true);
		imageStateThread.start();
	}

	public static void setIdleIcon() {
		if (!traySupported) return;
        synchronized(trayIcon) {
    		setImage(trayImages.idleImage);
    		imageStateThread = null;
        }
	}
	...
	public static void setErrorIcon() {
		if(!traySupported) return;
        synchronized(trayIcon) {
    		setImage(trayImages.errorImage);
    		imageStateThread = null;
        }
	}
	...
	public void setErrorMessage(String caption, String message, String transactionKey) {
		...
		try{
			if(!useSytemTrayPopups){
			    notificationQueue.getQueue().offer(new ToolTipReplacementFrame(popupDialogDisplayMsec, caption, message, trayImages.errorImage));
			}
	...
	public void setMessage(String caption, String message,String transactionkey) {
		...
		try{
			if(!useSytemTrayPopups) {
			    notificationQueue.getQueue().offer(new ToolTipReplacementFrame(popupDialogDisplayMsec, caption, message, trayImages.activeImage));
			}
			else if (traySupported && SystemTray.isSupported()) {
