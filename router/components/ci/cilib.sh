...
###################################################################
#
# Utilities for running continuous integration scripts
# for MedCommons Components
#
...
##
# Check if the log contains only changes that have paths matching the specified text
# $1  -   svn log file
# $2  -   text to match (passed to grep)
##
function svn_log_has_only_changes() {
  grep -v Restored "$1" | grep -v "At revision" | grep -v "Updated to" | grep -v "$2" | wc | grep -q "^[[:space:]]*0.*0.*0";
}

##
# Check if the log contains any changes at all
# $1  -   svn log file
##
function svn_log_has_no_changes() {
  grep -v Restored "$1" | grep -v "At revision" | wc | grep -q "^[[:space:]]*0.*0.*0";
}
