...
public class RegressionTest {
	...
	private DummyClass ensureAccountExists() throws Exception {
		DummyClass account = this.getAccount();
		if(account == null) {
			account = new DummyClass();
			account.setAccountID(accountID);
	...
	public void testLock() throws Exception {	
		DummyClass account = this.ensureAccountExists();
	...
	public void testConcurrency() throws Exception {	
		System.out.println("Info: Testing concurrency...");
		// This just checks that there are no exceptions
		final int numStepsOuter = 5;
		final int numStepsInner = 100;
		final int numThreads = 10;
		final int numUnclosed = 10;
		Thread[] threads = new Thread[numThreads];
		for(int i = 0; i < numThreads; i++) {
			Thread t = new Thread() {
				public void run() {
					try {
						for(int count = 0; count < numStepsOuter; count++) {
							for(int i = 0; i < numStepsInner; i++) {
							...
							// Select, leaving iterator open
							for(int i = 0; i < numUnclosed; i++) {
								store.select(DummyClass.class);
								Thread.yield();
							}
							// Just transaction
							store.beginTransaction();
							try {
								for(int j = 0; j < numStepsInner; j++) {
									RegressionBean bean = new RegressionBean();
									store.insert(bean);
									Thread.yield();
									store.delete(bean);
    ...
    private void testLargeLoop() throws Exception {
    	System.out.println("Info: Testing long iteration...");
    ...
    private void testRollback() throws Exception {
    	...
    	store.beginTransaction();
    	try {
    		account.setBalance(prevBalance + 100.0);
    		store.save(account);
    	} finally {
    		store.endTransaction(true);
    	}
    	account = this.getAccount();
    	if(prevBalance == account.getBalance()) {
    		System.out.println("Info: Rollback test PASSED.");
    	}
    	else {
    		System.out.println("ERROR: Rollback test FAILED.");
    	}
    }
