...
//	}
	...
	public int getMemoryObjectCount() {
		synchronized(this.objectsMonitor) {
			return this.objectInfosByObject.size();
		}
	}
...
//	}
	...
	/**
	 * Ends a transaction previously begun by a call to {@link #beginTransaction}.
	 * @param rollback Whether the transaction should be rolled back. 
	 */
	public void endTransaction(boolean rollback) throws StoreException {
		try {
			this.database.endTransaction(rollback);
		}
		catch(SQLException err) {
			throw new StoreException(err);
		}		
	...
	/**
	 * Attempt to get a lock for the given bean, using it's key as the locking data
	 * 
	 * @param bean             bean to lock
	 * @throws StoreException
	 * @throws InterruptedException
	 */
	public void requestLock(Object bean) throws StoreException {
		this.requestLock(bean.getClass(), String.valueOf(getPersistentID(bean)), Connection.TRANSACTION_READ_COMMITTED);
	}
	
	/**
	 * Request a lock for the class of the given bean using given data name
	 */
	public void requestLock(Object bean, String dataName) throws StoreException {
		this.requestLock(bean.getClass(), dataName, Connection.TRANSACTION_READ_COMMITTED);
	}
	 ...
	 * @see #requestLock(Class, String, int)
	 */
	public void requestLock(Class beanClass, String dataName) throws StoreException {
	 ...
	 */
	public void requestLock(Class beanClass, String dataName, int transactionIsolationLevel) throws StoreException {
	 ...
	 * {@link #requestLock(Class, String, int)}.  
	 * @throws StoreException
	 */
	public void relinquishLock() throws StoreException {
		this.relinquishLock(false);
	}
	 ...
	 * @throws StoreException
	 */
	public void relinquishLock(boolean rollback) throws StoreException {
		try {
			this.database.relinquishLock(rollback);
		} catch(SQLException se) {
			throw new StoreException(se);
		}				
	}
	...
	private ObjectInfo insertBeanTransac(Object persObject) throws StoreException {
		...
		try {
			// Lock order: Transactions should always be outside
			// of objectMonitor synchronization (?).
...
//	}
	 ...
	 * Gets an iterator of all stored instances of the given class.
	 */
	public ResultsIterator select (final Class inbeanClass, String property, Object value) throws StoreException {
		return select (inbeanClass, Collections.singletonMap(property, value));
	}
	 ...
	 * @throws StoreException
	 */
	public <T> T selectSingle (final Class beanClass, String propertyName, Object propertyValue) throws StoreException {
		Map values = new HashMap(2);
		values.put (propertyName, propertyValue);
		return (T)selectSingle (beanClass, values);				      
	}
	 ...
	 * @throws StoreException
	 */
	public <T> T selectSingle (final Class beanClass, Map values) throws StoreException {
	...
	public Object selectSingle (final Class beanClass, Map values, String orderBy, boolean descending) throws StoreException {
		...
		try {
		    ...
		    // If there were no more results then i.hasNext() closes the iterator already
		    // so we do not need / want to close it again (causes dirty exception in log)
		    if(result != null)
				i.close();
		}
		return null;
	...
	public <T> List<T> all(final Class<T> beanClass, Map values) throws StoreException {
		ResultsIterator<T> i = select (beanClass, values, null, false, null);
	...
	public <T> List<T> selectValues(String sqlQuery, Object... parameters) {
	    
	    final List<T> result = new ArrayList<T>();
	    try {
            final ResultsIterator<BeanWrapper> llbi = this.database.query(sqlQuery, parameters,  new BeanUnmarshaller() {
                @Override
		...
		try {
			...
			return new ResultsIterator() {
				...
				public void finalize() {
					// Do nothing here. The finalizer of ResultsIterator<BeanWrapper>
					// will take care of it. Note also that we cannot call close()
					// in a random thread.
				}
...
//	}
	...
	private Long insertBeanImpl(Object persObject) throws StoreException {
		...
		try {
			...
			try {
				...
				setBeanId(resultingID, persObject);
    ...
    private void setBeanId(Long id, Object bean) {
        try {
            Method m = bean.getClass().getMethod("setId", Long.class);
            if(m != null) {
                m.invoke(bean, id);
            }
        } 
	...
	private Object[] getParameters(StoreInfo storeInfo, Map values, boolean isPropertyValues, InsertionEntry currentInsertionEntry) throws StoreException {
		...
		while (i.hasNext()) {
			...
			if(rawValue instanceof IMultiValueContainer) {
				...
				Object value;
				if(isPropertyValues) {
				    if(rawValue != null) { // null values are tested with "IS NULL" so do not get a value in the params array
						String propertyName = (String) entry.getKey();
