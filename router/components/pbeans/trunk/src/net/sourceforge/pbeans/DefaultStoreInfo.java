...
class DefaultStoreInfo implements StoreInfo {
	...
	private FieldDescriptor getFieldDescriptorImpl(PropertyDescriptor pd, int maxColumnLength) throws StoreException {		
	    ...
	    if("metaClass".equals(pd.getName())) {
			if(logger.isLoggable(Level.INFO)) {
				logger.info("getFieldDescriptor(): Property " + pd.getName() + " of class " + this.clazz.getName() + " matches groovy metaClass and is therefore marked transient.");
			}
			return null;
	    }
		...
		if(getter == null || setter == null) {
		    ...
		    // SSADEDIN:  WHO CARES???? changed to INFO
			if(logger.isLoggable(Level.WARNING) && !"class".equals(pd.getName())) {
				logger.fine("getFieldDescriptor(): Property " + pd.getName() + " of class " + this.clazz.getName() + " does not have a getter and a setter.");
			}
			return null;
		}
