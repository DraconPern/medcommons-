...
public abstract class AbstractDatabase implements Database {
    ...
    protected void initDatabaseInfo() throws SQLException {
    	...
    	try {
    		...
    		this.releaseConnectionWrapper(cw);
    	}
    }
...
//    				}
    		...
    		while (checkI.hasNext()) {
    			...
    			// SSADEDIN
    			if(fd.getName().equals(mandatoryField.getName()))
    			    continue;
...
//    }
     ...
     * @throws SQLException
     */
    public final void relinquishLock(boolean rollback) throws SQLException {
    ...
    public ResultsIterator<BeanWrapper> query (String sql, Object[] parameters, BeanUnmarshaller unmarshaller) throws SQLException {
    	ConnectionPool.ConnectionWrapper cw = this.getConnectionWrapper();
  		PreparedStatement ps = cw.prepareStatement (sql);
   		for (int i = 0; i < parameters.length; i++) {
   			ps.setObject(i + 1, parameters[i]);
   		}
   		return getBeanIterator (cw, ps.executeQuery(), unmarshaller);
   		// Note that ConnectionWrapper is not released here because
    ...
    public Long insert(String tableName, String idField, String[] normalFieldNames, Object[] values, boolean autoIncrementRequested) throws SQLException {
    	...
    	try {
    		...
    		this.releaseConnectionWrapper(c);
    	}
    }
    	...
    	return new ResultsIterator<BeanWrapper>() {
    		...
    		public void finalize() throws Throwable {
    			...
    			if (this.hasNext()) {
        			...
        			try {
        				conn.release(ownerThread);
        			} catch(Exception err) {
           				logger.log(Level.WARNING, "close(): Unable to release connection", err);   				
        			}
    			}
    		}
    		...
    		public void close() {
    			...
    			try {
    				AbstractDatabase.this.releaseConnectionWrapper(conn);
    			} catch(Exception err) {
       				logger.log(Level.WARNING, "close(): Unable to release connection", err);   				
    			}
...
//    }
    ...
    protected void releaseConnectionWrapper(ConnectionPool.ConnectionWrapper wrapper) throws SQLException {
    	ConnectionPool.ConnectionWrapper tcwrapper = (ConnectionPool.ConnectionWrapper) this.transactionedConnection.get();
    	if(tcwrapper != null) {
    		if(wrapper != tcwrapper) {
    			throw new IllegalStateException("In transaction with " + tcwrapper + ", but attempting to release " + wrapper);
    		}
    		// We're in transaction - don't release the connection.
    		return;
    	}
    	wrapper.release();
    }
     ...
     * Ends a transaction.
     */
    public final void endTransaction(boolean rollback) throws SQLException {
    	ConnectionPool.ConnectionWrapper tcwrapper = (ConnectionPool.ConnectionWrapper) this.transactionedConnection.get();
    	if(tcwrapper == null) {
