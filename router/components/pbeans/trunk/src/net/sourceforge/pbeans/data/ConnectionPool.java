...
class ConnectionPool {
		...
		for (;;) {
			try {
				...
				if(logger.isLoggable(Level.FINE)) {
					logger.fine("getConnectionWrapper(): Creating connections.");
				}
	...
	class ConnectionWrapper implements Comparable {
		...
		private int prepareForUse(Thread t) {
			...
			// logger.warning("Connection " + this + " now owned by " + t);
			this.owner = t;
		...
		public void release() {
			...
			synchronized (monitor) {
				...
				// logger.warning("Connection " + this + " released from thread " + this.owner);
				this.owner = null;
