...
public class DatabaseImpl extends AbstractDatabase {
    ...
    protected PreparedStatement getCreateTableStatement (Connection c, String tableName, FieldDescriptor primaryFd, boolean autoIncrementRequested) throws SQLException {
    	return c.prepareStatement ("CREATE TABLE " + tableName + " (" + primaryFd.getName() + " " + getTypeDeclaration(primaryFd.getSqlType()) + (primaryFd.isNullable() ? " NULL" : " NOT NULL") + (autoIncrementRequested ? " AUTO_INCREMENT" : "") + (primaryFd.isPrimaryKey() ? " PRIMARY KEY" : "") + ") ENGINE=InnoDB");
    }
