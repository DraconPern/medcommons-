...
<xsl:template match="RegistryPackage">
	<xsl:variable name='id' select='@id'/>
	<li>
	<a href="/router/XdsQueryFoldersForContents?uuid={$id}&amp;stylesheet=FolderNodes.xsl">
...
<xsl:template match="ExtrinsicObject">
	...
	<xsl:variable name="docType">
			<xsl:choose>
				<xsl:when test="contains($mimeType,'pdf')">PDF</xsl:when>
				<xsl:when test="contains($mimeType,'ccr')">CCR</xsl:when>
				<xsl:when test="contains($mimeType,'cda')">CDA</xsl:when>
				<xsl:otherwise>Unknown document type</xsl:otherwise>
			</xsl:choose>
	</xsl:variable>
	<ul>
		<li> <xsl:value-of select='$docType'/> 
		 <a href="/router/XdsQueryObjectForExternalLinks1804?stylesheet={$stylesheet}&amp;id={$id}"> 
			<xsl:apply-templates select="Name"/> 
		</a> from 
		<xsl:apply-templates select="Slot[@name='authorInstitution']"/>/
		<xsl:apply-templates select="Slot[@name='authorDepartment']"/>/
		<xsl:apply-templates select="Slot[@name='authorPerson']"/>
		<br/>
...
"CCR/IHE 0.9"
-->
<xsl:template match="Classificationxx">
