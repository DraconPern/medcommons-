...
DocumentThumbnail.prototype.display = function() {
  ...
  if((this.series.mimeType=='application/pdf') && ((this.series.instances[0].FileReferenceID.indexOf("http://") != -1))) {
    documentUrl = this.series.instances[0].FileReferenceID;
  }
  else 
  if((this.series.mimeType=='application/x-hl7') && ((this.series.instances[0].FileReferenceID.indexOf("http://") != -1))) {
    documentUrl = this.series.instances[0].FileReferenceID;
    $('mainImage').style.backgroundColor='white';
  }
  else {
...
DocumentThumbnail.prototype.displayThumbnail = function(thumbIndex) {
  ...
  var thumbDescription = this.series.instances[0].FileReferenceID + ' ('+ this.series.SeriesDescription +')'
  if(
...
function toggleZoomWL(){
  ...
  if(currentToolState.currentWindow == null) {
    var image= p.studies[0].series[currentSeries].instances[currentImage];
    if(image != null) {
...
function updateDisplayedThumbnail(){
    ...
    var thumbImage= p.studies[0].series[currentSeries].instances[currentImage];
    var thumbURL = "wado/"+p.studies[0].series[currentSeries].storageId+"?studyUID=";
    thumbURL+=p.studies[0].StudyInstanceUID;
    thumbURL+="&rows=140&columns=140&fname=";
    thumbURL+=thumbImage.FileReferenceID;
    thumbURL+="&mcGUID=";
    thumbURL+=p.studies[0].series[currentSeries].mcGUID;
    thumbURL+="&windowCenter=";
    thumbURL+=thumbImage.defaultLevel;
    thumbURL+="&windowWidth=";
    thumbURL+=thumbImage.defaultWindow;
...
function Initialize() {
  ...
  */
  ...
  if(currentSeries >= 0) {
    ...
    selectSeries(p.studies[0].series[currentSeries]); // Was set in URL.
    setZoomActive();
    displayCurrentImage();
  }
...
function displayCurrentImage() {
    ...
    var series = p.studies[0].series[currentSeries];
    if(series.mimeType == 'application/dicom') {
      ...
      var image= series.instances[currentImage];
    ...
    // p.studies[0].series[currentSeries].instances[0].display();
    thumbnails[currentThumb].display();
  }
  adjustContentAreaSize();
}
...
function getImageUrl(urlSeries,urlImage,frameNumber,quality) {
     ...
     var imageURL = "wado/" + p.studies[0].series[currentSeries].storageId + "?"; 
     imageURL += queryString( {
       studyUID: p.studies[0].StudyInstanceUID,
       fname: image.FileReferenceID,
       patientName: p.PatientName,
       mcGUID: p.studies[0].series[currentSeries].mcGUID
     });
...
function resetSeriesChange(){
  ...
  currentThumb = thumbnailIndexFromSeries(p.studies[0].series[currentSeries]);
...
function xdsLoaded() {
  if(isSubmitting) {  // Submit must be finished.
    ...
    p.studies[0].series.splice(
      0,0,new Series("MedCommons Document*", "new-cda", "new-cda", "", -1)); 
    p.studies[0].series[0].instances[0]=
      new Instance( "new-cda-instance", "0", "showReport.jsp?stylesheet=cda2htm&source=newCda", 0, 0);
      p.studies[0].series[0].mimeType="text/x-cdar1+xml";
    thumbnails.splice(1,0, new DocumentThumbnail(p.studies[0].series[0])); 
    ...
    p.studies[0].series[0].mimeType="application/x-ccr+xml";
    thumbnails.splice(1,0, new DocumentThumbnail(p.studies[0].series[0]));
