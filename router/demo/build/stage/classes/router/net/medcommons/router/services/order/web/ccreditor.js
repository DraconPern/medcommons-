...
function addDicomReference(){
  ...
  // If DDL identity is not initialized, query it and recall this
  // function afterwards (may cause prompt to user).
  if(!ddlId) {
      log("DDL identity not know, querying");
      queryDDLID(addDicomReference);
      return;
  }
  ...
  paymentRequiredDlg(evt, function() {
    execJSONRequest('Billing.action',{ccrIndex: parent.currentTab.ccrIndex,type:'DICOM',count:1},function(result) {
      ...
      sendCommand('quickupload', {
              auth: auth,
              gwUrl: remoteAccessAddress,
              groupname: groupName,
              groupAccountId: groupId,
              accountid: accId,
              storageid: patientMedCommonsId.replace(/ /g,''), 
              voucher: false,
              documentType: parent.currentTab.ccr.documentType,
              jsonp: 'dicomUploadStarted'});
              
      connect(commands,'quickuploadComplete', function(result) {
          if(result.importStatus.status == "Error") {
              alert("A problem occurred uploading DICOM to this patient:\n\n" + result.importStatus.message);
              return;
          }
          ...
          // Force reload of window in firefox. parent.location.reload() appears to have some other side effects
          // (such as invoking addDicomReference() a second time).
          parent.location.href = 'view#'+parent.currentTab.ccrIndex+'e';
	  });
    });
  });
}

function dicomUploadStarted() {
   log("DICOM upload started"); 
}
...
function addWebRefSubmit() {
  var url = 'AddWebReference.action?ccrIndex='+window.parent.currentTab.ccrIndex;
  log('adWebRefSubmit loading url ' + url);
  execJSONRequest(url,queryString(document.addWebReferenceForm),function(result) {
      if(result.status == "ok") {
        refreshPage(); 
      }
      else
...
function fetchRegistryPatients() {
  ...
  hide('patientFindImg');
  loadJSONDoc('FetchRegistryPatients.action',{ccrIndex:window.parent.currentTab.ccrIndex, sid:getCookie("JSESSIONID")}).addCallbacks(fetchRegistryPatientsSuccess,XHRErrorHandler);
}
