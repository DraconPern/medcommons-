...
; 
; This installer script creates an entire distribution containing the MedCommons Router,
; System Monitor and GUI client (selection screen).
;
; The distribution includes all of Tomcat and is thus quite large.
; 
; $Id: medcommons.nsi 636 2005-05-13 08:24:47Z ssadedin $
;
;--------------------------------
...
;Configuration
  ...
  ;General
  Name "MedCommons"
  Caption "MedCommons Setup Version ${VERSION} (Revision #${REVISION})"
...
found.NETFramework:
  ...
  File bin\Interop.stdole.dll
  File bin\Common.dll
  File bin\AxInterop.SHDocVw.dll
  File bin\Interop.SHDocVw.dll
  File bin\microsoft.mshtml.dll
...
Section "MedCommons Router Software" SecRouter
  ...
  !else
    SetOutPath "$INSTDIR\Router\tomcat"
    CreateShortCut "$STARTMENU\Programs\MedCommons\Start MedCommons Router.lnk" "$INSTDIR\Router\tomcat\bin\catalina.bat" "run"
  !endif
  SetOutPath "$INSTDIR"
SectionEnd
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\MedCommons"
