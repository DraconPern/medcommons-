...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xdt="http://www.w3.org/2005/xpath-datatypes">
	...
	<xsl:template match="/">
		<ContinuityOfCareRecord xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="urn:astm-org:CCR">
			...
			<Purpose>
				<Description>
					<Text>FAX Transmission <xsl:value-of select="*/FaxControl/MCFID"/>/<xsl:value-of select="*/FaxControl/FaxName"/></Text>
				</Description>
			</Purpose>
			<Body>
			</Body>
						...
						<Value>
							...
							<xsl:call-template name="format-telephone">
								<xsl:with-param name="telephone-number">
									<xsl:value-of select="*/FaxControl/ANI"/>
								</xsl:with-param>
							</xsl:call-template>
						</Value>
						<Type>
				...
				<Actor>
					...
					<Telephone>
						<Value>
							...
							<xsl:call-template name="format-telephone">
								<xsl:with-param name="telephone-number">
									<xsl:value-of select="*/FaxControl/AccountID"/>
								</xsl:with-param>
							</xsl:call-template>
						</Value>
						<Type>
