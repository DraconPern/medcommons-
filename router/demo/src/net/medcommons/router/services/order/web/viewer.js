...
/**
 * Shorthand for YAHOO.util.Element.get
 */
function el(id) {
...
function ccrNode(pf) {
    ...
    var div = DIV({'class':'encounterRow'},IMG({src:'images/folder.gif'}),title);
    div.profile = pf;
    
    Y.one(div).on('click', expandEpisode);
    return div;
}

function expandEpisode(e) {
  alert('Expanding ' + e.target._node.profile.documentType + ' due to ' + e.type);
}
