...
var ddlQueryIDNext = null;
function queryDDLID(next) {
    if(ddlId != null) 
        return;
    ddlQueryIDNext = next;
    sendCommand("queryid", {ddlid: ddlId,  jsonp: 'onQueryIdResponse' });
}
...
function onQueryIdResponse(result) {
    ...
    setCookie('ddlid', ddlId);
    if(ddlQueryIDNext) {
        var n = ddlQueryIDNext;
        ddlQueryIDNext=null;
        n();
    }
}
...
function sendCommand(cmd, opts) {
  ...
  // If possible, use the proxy
  if(commandProxyBaseURL && ddlId && cmd != "ping" && cmd != "queryid")
      url = commandProxyBaseURL;
  
  loadContextManagerURL(url + "?command="+cmd + '&' + args.join('&'));
}
