...
function tabDateTitle(profile) {
  if(profile.createDateTime) {
      return profile.createDateTime.replace(/ .*$/,'');
  }
  else
  if(typeof profile.date == 'number') {
    return toAmericanDate(new Date(profile.date)).replace(/\//g," / ");
  }
  else 
    return toAmericanDate(profile.date).replace(/\//g," / ");
}
...
/**
 * Examines each profile and maps it to one of the 
 * known logical document types if the guid or name
 * matches.
 * 
 * @param profiles
 * @return
 */
function addDocumentInfo(profiles) {
    var ccrs = [];
    log("====> Adding CCRs " + profiles);
    var logicalDocuments = new Array();
    forEach(profiles, function(entry) {
      ...
      for(var t in accountDocuments) {
        ...
        if(((entry.guid && (d.guid == entry.guid)) || (entry.name == d.type)) && !logicalDocuments[d.type]) {
    ...
    forEach( [ 'NEWCCR','EMERGENCYCCR','CURRENTCCR' ], // note: reverse order to how we want them to appear
      function(docType) {
        if(logicalDocuments[docType]) {
            ...
            ccrs.splice(0,0,logicalDocuments[docType]);
        }
    });
...
function addPatientTabs() {  
  //try {
    ...
    var numTabs = Math.floor(document.body.scrollWidth/(12*11+13)) - 1; // subtract one to leave room for new tab
...
function quickShare() {
    var div;
    lightbox(div = DIV({id:'quickShareDlg'}), {w:620,h:400});
    doSimpleXMLHttpRequest('QuickReply.action?share&ccrIndex='+parent.currentTab.ccrIndex).addCallbacks(function(r) {
        $('notificationsTableCache').innerHTML = r.responseText;
        quickReply(div,function(dlg) {
            dlg.style.left = '0px';
            dlg.style.top = '0px';
            replaceChildNodes(dlg.getElementsByTagName('h4')[0],SPAN({style: 'padding-left: 1em;'},"Quick Share"));
            var toEmail = $$('#quickShareDlg #toEmail')[0];
            toEmail.value = '';
            $('replyComment').value = 'Type a message here';
            connect('replyComment','onfocus',function() { this.select(); } );
            return toEmail;
        }, true);
    }, genericErrorHandler); 
}

function quickReply(container,init,enabled) {
  if(!container) 
  ...
  var dlg = DIV({'class':'dialog',id:'quickReplyDlg'}, 
              hd = DIV({'class':'hd'},createDOM('H4',null,SPAN({style: 'padding-left: 1em;'},"Quick Reply"))),
              DIV({'class':'dlgContent', style:'border: 1px solid #898770;'},  
                  ...
                  DIV({id:'replyStatus', style:'height: 2.0em;'})
                  )
      );
  
  roundElement(hd,{corners:'top', color: '#898770'});
  hd.getElementsByTagName('div')[0].style.backgroundColor = 'transparent';
  forEach(hd.getElementsByTagName('span'),function(s) { s.style.borderColor = 'transparent'; })
  
  appendChildNodes(container,dlg);
  
  $('notifications').innerHTML = $('notificationsTableCache').innerHTML;

  // Before displaying, give opportunity to initialize
  var toFocus;
  if(init) 
      toFocus =init(dlg);
  
  if(!toFocus)
       toFocus = $('replyComment');
  ...
  connect(cbt,'onclick',function() { removeElement(dlg); if($('underlay')) removeElement('underlay'); });
  
  if(!enabled)
	  forEach($('notifications').getElementsByTagName('input'), function(i) { i.disabled = true; });
  
  connect(bt,'onclick', function() {
      var toEmail = $$('#quickReplyDlg #toEmail')[0]; 
      ...
      bt.disabled = true;
      execJSONRequest('QuickReply.action',queryString({ccrIndex:$('ccrTableCCRIndex').value,
          toEmails: toEmail.value,
          sourceEmail: $('sourceEmail').value,
          comment:$('replyComment').value}), function(result) {
        clearInterval(i);
        if(result.status == 'ok') {
          ...
          bt.innerHTML='Close';
          disconnectAll(bt);
          connect(bt,'onclick',function() { removeElement(dlg); if($('underlay')) removeElement('underlay'); });
        }
        else {
...
function dialog(id, header, contents, width, buttons, init) {
  ...
  if(typeof buttons.length == 'undefined') { // Not an array
      var arr = [];
      for(i in buttons) {
          log("Found button " + i);
          arr.push({text: i, handler: buttons[i]});
      }
      buttons = arr;
  }
  ...
  var createDlg = function() {
      hide('underlay');
      if($('waitMsg'));
          removeElement('waitMsg');
      ...
      if(typeof contents != 'string') {
          replaceChildNodes(dlg.body, contents);
      } 
  ...
  lightbox(DIV({id: 'waitMsg', style:'border: 2px black; font-size: 9px; padding: 5px; background-color: white;'},'Please Wait ...'));
  yuiLoader().insert(createDlg);
}
...
function authenticateImport() {
  execJSONRequest('AccountImport.action?authenticate=true', queryString($('accountImportForm')), function(result) {
     if(result.status == 'ok') {
       window.location='AccountImport.action?displayConfirm=true&' + queryString({
           sourceUrl: result.sourceUrl,
           sourceAuth: result.token,
           toAccount: ccr.patient.medCommonsId
       });
     }
     else {
...
function mergeToHealthURL() {
    dialog('mergeDlg', 'Merge to HealthURL', 
            'You can merge contents from this CCR into another account.  You must have consent to '
            + 'modify the other account.<p>'
            + 'Enter the full HealthURL or MedCommons Account Id for the other account below:</p>'
            + '<form id="healthURLMergeForm">'
            + '<table><tr><th>HealthURL or ID</th><td><input type="text" id="targetUrl" style="width:300px;"/></td></tr>'
            + '<tr><td> </td><td class="hurlExample">Example:  '+accountsBaseURL+'0123456789012345</td></tr>'
            + '</table></form>',
            420, // width
            [ 
             { text: 'Merge', handler: function() {
                   var targetHealthURL = $$('#mergeDlg #targetUrl')[0].value; 
                   if(!targetHealthURL.match(/(http[s]:\/\/.*\/){0,1}[0-9]{16}/)) {
                       alert('The text you entered did not appear to be a valid HealthURL or 16 digit MedCommons Account ID');
                       addElementClass($$('#mergeDlg #targetUrl')[0], 'invalid');
                       return;
                   }
                   window.location='AccountImport.action?displayConfirm=true&' + queryString({
                       toAccount: targetHealthURL.replace(/[^0-9]/g,''),
                       sourceAuth: auth,
...
function warnIncompletePatient(next) {
    dialog('incompletePatientWarning','Incomplete Patient Data',
          ...
          +'and visible data may change as further content is uploaded.</p>', 460, 
          { OK : function() {this.destroy(); if(next) next();}}, function(dlg) {
      dlg.cfg.setProperty("icon",YAHOO.widget.SimpleDialog.ICON_WARN);
    });
}
function showStartDDLHelp() {
    alert(
            'No DDL service could be detected on your computer.\n\n' +
            'To send or receive DICOM you need to start the DDL DICOM Service.\n\n'+ 
            'Click OK to open the DDL control page to start or install DDL.  Once the DDL DICOM service ' +
            'is running on your computer you can return to this page and try your operation again.'
         );
    var host =  (typeof acctServer == 'undefined') ? parent.acctServer : acctServer;
    window.open(host + '/../ddl.php'); 
}
function downloadAllDocuments() { 
    
    var msg = '<h4>Health records can contain sensitive information</h4>'
              +'<p>If you download sensitive information to a shared or unsecured computer or device, others might see it.</p>'
              +'<p>You are responsible for protecting the information you download, and for deciding with whom to share it.</p>'
              +'<p>Are you sure you want to download a copy of your personal health information to the computer or device you are using?</p>';
    
    dialog('downloadAllDocumentsWarning','Download Documents',
           msg, 460, {
            'Download': function(dlg) {
               load('PersonalBackup?'+queryString({storageId: storageId,auth:auth}));
               this.destroy();
            },
            'Cancel': function(dlg) { this.destroy(); },
            'Tell Me More': function() {window.open(accountsBaseURL+'downloaddocuments.php'); this.destroy();}
       });}
...
function submitPhone(patientId) {
    ...
    try {
        ...
        log('Testing validation of phone fields');
        ...
        log('disabling fields');
        forEach(validations, function(v) { v[0].disabled = true; });
    
        log('sending phone share reques');
...
function sharePhone(patientId) {
    ...
    yuiLoader().insert(function() {
      ...
      var dlg = new YAHOO.widget.SimpleDialog("phoneDlg", Y.lang.merge(dialogDefaults,{ 
        buttons: [
                  { text: 'OK', isDefault: true, handler: partial(submitPhone,patientId) }, 
                  { text: 'Cancel', handler: function() { dlg.destroy(); } } 
                 ]
      }));

      dlg.setHeader("Share by Phone");
      dlg.setBody("<p>Share this CCR with another person by entering their name and phone number below:</p>"
        + "<form id='phoneForm'>"
