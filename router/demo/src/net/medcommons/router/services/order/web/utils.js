...
function isValidEmail(value) {
    return value.match(/\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}\b/i) != null;
}
...
function yuiLoader() {
    ...
    if(!window.loader) {
      ...
      loader.insert({'base':'yui-2.8.2r1/'});
    }
...
if(typeof getElementDimensions == 'undefined' && typeof YAHOO.util.Dom != 'undefined') {
    window.getElementDimensions = function(e) { 
        var r = YAHOO.util.Dom.getRegion(e);
 ...
 * YUI (usese MochiKit)
 */
function lightbox(body,dim,opacity) {
    ...
    underlay.setAttribute("style",'position: absolute; top: 0px; left: 0px; width: 100%; height: 100%; background-color: black; z-index: 100000; display: none;');
...
function isMobileBrowser() {
    if(typeof YAHOO == 'undefined')
        throw "Depends on YUI - please include it";
    
    if(YAHOO.env.ua.mobile)
        return true;
    else
    if((YAHOO.env.ua.mobile == null) 
            && (YAHOO.env.ua.os == null) 
            && (YAHOO.env.ua.webkit > 0) 
            && (window.navigator.appName=='Netscape')
            && (window.navigator.platform.match(/^Linux.*arm.*$/))
        ) // Android
        return true;
...
/**
 * Show a timed message at the top left corner of the element with specified.
 * The message will automatically disappear after a few seconds.
 * 
 * @param id    id of parent element
 * @param msg   message to show
 * @requires    Mochikit DOM
 */
function timedMessage(id, msg) {
    var div=$('messageDiv');
    if(!div) 
        appendChildNodes(id, div=DIV({ 
            id: 'messageDiv', 
            style: 'position: absolute; top: 0px;  padding: 3px 4px; color: white; background-color: blue; z-index: 1000000'
        }));
    if(div.timer)
        clearTimeout(div.timer);
    div.timer = setTimeout(function() { removeElement('messageDiv'); }, 3000);
    replaceChildNodes('messageDiv', SPAN(msg));
}
