...
<#assign head>
  ...
  <script type='text/javascript'>
    ...
    function open_target_hurl() { 
      ...
      if(sourceWindow)
        sourceWindow.close();
      sourceWindow = window.open('CurrentCCR.action?m=editdg&a=${action.toAccount?js_string}&auth=${desktop.authenticationToken?js_string}');
      return false;
    }
    var accid = '${action.toAccount?js_string}';
...
<@basic_layout title="Import / Merge Account" head=head onload='init();'>
  <#escape x as x?html>
    <div id='content'>
      ...
      <#if !action.hasValidationErrors >
        ...
        <form name='importForm' action='AccountImport.action'>
          ...
          <input type='hidden' name='toAccount' value='${action.toAccount?default("")}'/>
        </form>
        <hr/>
      </#if>
      </div>
    </#escape>
    <script type='text/javascript' src='yui-2.8.2r1/yuiloader/yuiloader-min.js'></script>
    <@pack.script>
      <src>utils.js</src>
      <src>common.js</src>
