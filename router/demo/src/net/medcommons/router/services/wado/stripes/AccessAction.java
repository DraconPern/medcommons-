...
public class AccessAction implements ActionBean, ValidationErrorHandler {
    ...
    /**
     * If set to true, share form will be invoked on load
     */
    private boolean share = false;
    
    /**
     * Format parameter - "json", "xml" and "full" are recognized values,
     * note they can also be set as separate parameters, see above.
     * "full" causes the full version to show even when a mobile viewer
    ...
    public Resolution access() throws Exception {
        try {
            ...
            String ctxAuth = null;
            session = UserSession.get(this.ctx.getRequest());
            if(session != null && !blank(session.getContextAuth())) {
                ctxAuth = session.getContextAuth();
            }
            
            this.session = UserSession.clean(this.ctx.getRequest(), this.a, this.at);
            if(ctxAuth != null) 
                session.setContextAuth(ctxAuth);
            ...
            if(share) 
                ctx.getRequest().getSession().setAttribute("share", Boolean.TRUE);
    ...
    private Resolution getResolution() throws PHRException, IOException, ServiceException {
        ...
        if(useMobileDisplay()) 
            return new ForwardResolution("/viewer");
        
        return new ForwardResolution("/platform.jsp");
    }

    protected boolean useMobileDisplay() {
        String ua = ctx.getRequest().getHeader("User-Agent");
        return ("mobile".equals(fmt) || ua.indexOf("iPhone")>=0 || ua.indexOf("iPad")>=0) && !"full".equals(fmt);
    }
    ...
    private Resolution loadCcr(boolean failIfNotFound) {
        ...
        try {
            ...
            if(!blank(ccr.getPatientMedCommonsId())) {
	            ctx.getRequest().getSession().setAttribute("expiryDate",session.getAccountSettings(ccr.getPatientMedCommonsId()).getExpiryDate());
            }
            return null;
        }
