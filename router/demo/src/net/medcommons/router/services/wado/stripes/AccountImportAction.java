/*
 * $Id: AccountImportAction.java 3807 2010-08-18 08:32:48Z ssadedin $
...
public class AccountImportAction extends BaseActionBean implements ValidationErrorHandler {
    ...
    /**
     * Account settings for the target into which content is being
     * imported.  If available the target CCR is preferred as a 
     * source of patient data (name, etc.), but if the target user
     * has no CCR then the settings will be used in their place.
     */
    private AccountSettings targetSettings;
    ...
    /**
     * Resolve source and target information to prepare for merge / import
     * 
     * @throws Exception
     */
    private void initialize() throws Exception {
        ...
        if(!session.checkPermissions(toAccount, Rights.WRITE.name()))
            throw new InsufficientPrivilegeException("Your current session is not authorized to modify account " + toAccount);
        
        if(!blank(toAccount)) {
            targetSettings = session.getAccountSettings(toAccount);
            String guid = targetSettings.getCurrentCcrGuid();
            this.targetCCR = session.resolve(guid);                            
        }
        else {
            this.targetCCR = session.getLogicalCCR(AccountDocumentType.CURRENTCCR);
            targetSettings = session.getAccountSettings(toAccount);
            toAccount = session.getOwnerMedCommonsId();
        }
    ...
    /**
     * Check the target and source CCRs / patients to see if they have
     * conflicting attributes (such as name, date of birth, etc.).
     * <p>
     * Mismatches found are added to the {@link #mismatches} field
     */
    private void computeMismatches() throws MergeException, ServiceException, PHRException {
        if(targetCCR != null) {
            ...
            AccountSettings settings = targetSettings;
    ...
    public Resolution showConfirm() throws Exception {
        ...
        // We copy the reference from the first session instance in case the user refreshes
        // the browser.
        httpSession.setAttribute("importAction",importActionBean);
        httpSession.setAttribute("initialContentsUrl", "AccountImport.action?renderConfirm");
    ...
    /**
     * Render the confirmation page - no initialization
     * @return
     */
    @DontValidate
    public Resolution renderConfirm() { 
        HttpSession s = ctx.getRequest().getSession();
        if(s.getAttribute("importAction") == null) {
            s.setAttribute("importAction",s.getAttribute("importActionBean"));
        }
        return new ForwardResolution("/importAccount.ftl"); 
    }
    
    /**
     * Entry point for showing page directly (eg: when 
     * already targetting content frame)
     */
    public Resolution displayConfirm() throws Exception {
        try {
            ...
            return new ForwardResolution("/mergeNoConsent.ftl");
        }
        return new ForwardResolution("/importAccount.ftl");
    }
    ...
    private void mapAndStoreCCR(String targetAccount, String fromAccount, String ccrXML) throws AccountImportException, MergePolicyViolationException {
        try {
            ...
            if(blank(this.session.getAccountSettings(tx.getPatientAcctId()).getCurrentCcrGuid())) {
               tx.merge(); 
            }
            else {
                // Save as a tab
                profileService.createProfile(tx.getPatientAcctId(), new PHRProfile(tx.getDocumentGuid()));
            }
            checkCancelled();
        }
    ...
    public AccountSettings getTargetSettings() {
        return targetSettings;
    }

    public void setTargetSettings(AccountSettings targetSettings) {
