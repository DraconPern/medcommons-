...
public class DisplayCCRAction extends CCRActionBean {
	...
	public Resolution body() throws Exception { 
	    
	    String stylesheet = "ccr2htmbody.xsl";
	    
	    log.info("Rendering only body of CCR");
	    
        return renderHTML(stylesheet);
	}
	
	@DefaultHandler
	public Resolution display() throws Exception { 
	    
	    String stylesheet = "ccr2htm.xsl";
	    
        return renderHTML(stylesheet);
	}

    private Resolution renderHTML(String stylesheet) throws RepositoryException, ParseException,
                    PHRException, IOException, TransformerFactoryConfigurationError, Exception {
        ...
        log.info("displaying CCR [specified guid = " +guid+ " default guid = " + ccr.getGuid() + "] stylesheet="+stylesheet);
        ...
        try {
	        ...
	        if(blank(ccr.getGuid()) || ccr.getJDOMDocument().getModified() || hackForceNoCompression) {
		        ...
		        JDOMResult out = doTransform(stylesheet);
		        new XMLOutputter(utfOutputFormat).output(out.getDocument(), response.getOutputStream());
		        return null;
	        }
	        else {
	            // cache the HTML
		        String fileName = "data/repository/"+ccr.getStorageId()+"/"+ccr.getGuid() + "_"+ stylesheet.replace('.', '_')+".html.gz";
     ...
     */
    private JDOMResult doTransform(String stylesheet) throws TransformerConfigurationException, TransformerFactoryConfigurationError,
                    PHRException, TransformerException {
        log.info("Creating transformer");
        StreamSource source = null;
        source = new StreamSource(new File("data/stylesheets/"+stylesheet));
