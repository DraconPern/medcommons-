...
public class CCRPutAction extends BaseActionBean implements ValidationErrorHandler {
    ...
    private void importImageFile(File imgFile, String contentType) throws Exception {
        ...
        if(ccr.findReference(descr.getGuid()) == null) { 
	        ...
	        session.getServicesFactory().getDocumentService().addDocument(storageId, series.getMcGUID());
    ...
    private File decodeImage() throws UnsupportedEncodingException, IOException {
        ...
        if(gzip) {
            log.info("Input base64 encoded data was gzipped");
            InputStream in = new GZIPInputStream(new ByteArrayInputStream(imgBytes));
            OutputStream out = null;
            try {
            ...
            log.info("Unzipped image file:  input length = " + imgBytes.length + " output length = " + imgFile.length() 
                     + " ratio = " + ((float)imgBytes.length) / ((float)imgFile.length()+1));
        }
        else {
            // Save the image
