...
public class WADOImage2 {
   ...
   * This can be overridden on an image by image basis - see
   * {@link WADOParameterForm#maxSpriteHeight}
   */
  ...
  /**
   * Width of individual frame in output result
   */
  private int frameWidth = Integer.MIN_VALUE;  
  
  /**
   * Height of inidividual frame in output result
   */
  private int frameHeight = Integer.MIN_VALUE;  
   ...
   */
  protected int columns = Integer.MIN_VALUE;
  
  private int numberOfFrames = Integer.MIN_VALUE;
  
  private double scale = 1.0;  
  
  protected  DicomObject ds = null;
  ...
  public double calculateScale(int maxColumns, int maxRows, int numFrames) {
    ...
    if((this.columns != Integer.MIN_VALUE) && (this.rows != Integer.MIN_VALUE)) {
      ...
      this.outputHeight = this.rows*numFrames;
      this.frameHeight = this.rows;
      this.frameHeight = this.columns;
    } 
    else
    if((maxColumns != Integer.MIN_VALUE) && (maxRows != Integer.MIN_VALUE)) {
      ...
      this.frameWidth = this.outputWidth;
      this.frameHeight = (int) (this.originalHeight * scale);
    }
    else
  ...
  /**
   * Anything set in here will be added as a Cookie to the output response
   */
  ...
  protected DicomImageReadParam dicomParam;
  ...
  public void createImage(WADOParameterForm params) {
      try {
          ...
          if(params.isFrameSprite()) {
              image = renderMultiframeSprite(params, originalImage, dicomParam);
          }
          else {
	    	  image=originalImage;
          }
  ...
  /**
   * Extract an image from the DICOM file for this image
   * 
   * @param params
   * @return
   * @throws IOException
   */
  protected BufferedImage extractImage(WADOParameterForm params) throws IOException {
      dicomParam = (DicomImageReadParam) reader.getDefaultReadParam();
      dicomParam.setWindowCenter(params.getWindowCenter());
      dicomParam.setWindowWidth(params.getWindowWidth());
      specifiedWindowCenter = params.getWindowCenter();
      specifiedWindowWidth = params.getWindowWidth();
      
      log.info("W/L = " + specifiedWindowWidth + " / " + specifiedWindowCenter);
      
      String pmi = params.getPhotometricInterpretation();
      monochrome = isMonochrome(pmi);
      return reader.read(params.getFrameNumber(), dicomParam);
  }
    

  /**
   * Render an image containing multiple consecutive frames of the 
   * DICOM image positioned in a vertical strip.
   * <p>
   * The height of the image will be limited such taht it does not exceed
   * MAX_SPRITE_HEIGHT after the requested scaling for this {@link WADOImage2} 
   * has been applied (this method does not apply the scaling itself).  The
   * number of frames rendered will be adjusted so that it fits within
   * this height.
   * 
   * @param params           input parameters (eg: from web)
   * @param originalImage    the first frame, already rendered
   * @param param            DICOM parameters (from dicom file)
   */
  private BufferedImage renderMultiframeSprite(WADOParameterForm params, BufferedImage originalImage,
                DicomImageReadParam param) throws IOException {
      ...
      int maxFrames = (int)Math.floor((double)params.getMaxSpriteHeight() / frameHeight);
      ...
      // this is the wrong height ... the frameHeight is the output height - it should be scaled by
      // the same amount as the image will later be scaled
      int preScaleFrameHeight = (int) (frameHeight/scale);
      BufferedImage sprite = 
          new BufferedImage((int)region.getWidth(), (int) (numFrames*preScaleFrameHeight), imageType);
      ...
      for(int i=1; i<numFrames; ++i) {
          ...
          gfx.drawImage(img.getSubimage(
              ...
              0,i*preScaleFrameHeight, observer);
      }
      log.info("Created sprite "+region.getWidth()+" x " + preScaleFrameHeight*numFrames + " (" + numFrames + " frames) in "
   ...
   * 
   * @param isFrameSprite      true if a sprite containing multiple frames 
   *                           is being rendered
   */
  public void scaleImage(int interpolationType, int maxColumns, int maxRows,
      ...
      if (interpolationType == WADOConstants.INTERPOLATION_FAST) {
        ...
        int renderWidth = (int) (scale * this.getOriginalWidth());
        int renderHeight = (int) (scale * this.getOriginalHeight());
        if(isFrameSprite)
            renderHeight = this.numberOfFrames * this.frameHeight;
