/*
 * $Id: CStoreSCU.java 60 2004-04-28 14:53:59Z sdoyle $
 * Created on Apr 21, 2004
 *
 * To change the template for this generated file go to
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
package net.medcommons.simulator.util;
 ...
 */
public class CStoreSCU implements PollDirSrv.Handler {
	...
	// Attributes ----------------------------------------------------
	static final Logger log = Logger.getLogger("CStoreSCU");
	...
	private static void set(DICOMProperties cfg, String s) {
		int pos = s.indexOf(':');
		if (pos == -1) {
			cfg.put("set." + s, "");
		} else {
...
//TODO: Move main into ModalitySimulator package.
	

	// Constructors --------------------------------------------------

	public CStoreSCU(DICOMProperties cfg, DcmURL url, int argc) {
		this.url = url;
	...
	private void initOverwrite(DICOMProperties cfg) {
		for (Enumeration it = cfg.keys(); it.hasMoreElements();) {
			String key = (String) it.nextElement();
			if (key.startsWith("set.")) {
				try {
					...
					throw new IllegalArgumentException(
						"Illegal entry in CStoreSCU.cfg - "
							+ key
							+ "="
							+ cfg.getProperty(key));
				}
	...
	private boolean initPollDirSrv(DICOMProperties cfg) {
	...
	private void initTLS(DICOMProperties cfg) {
		try {
