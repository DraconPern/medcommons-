...
def req_req(request):
	...
	if form.is_valid():
	    cert = Certificate()
	    cert.CN = form.cleaned_data['CN']
	    cert.C  = form.cleaned_data['C']
	    cert.ST = form.cleaned_data['ST']
	    cert.L  = form.cleaned_data['L']
	    cert.O  = form.cleaned_data['O']
	    cert.OU = form.cleaned_data['OU']

	    cert.key = genrsa(form.cleaned_data['pw1'])
	    cert.issued = datetime.now()

	    subject = form.cleaned_data.copy()
	    del subject['pw1']
	    del subject['pw2']
