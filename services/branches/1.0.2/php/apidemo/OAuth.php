...
class OAuthSignatureMethod_HMAC_SHA1 extends OAuthSignatureMethod {/*{{{*/
  ...
  public function build_signature($request, $consumer, $token) {/*{{{*/
    ...
    $sig = array(
      rawurlencode($request->get_normalized_http_method()),
      rawurlencode($request->get_normalized_http_url()),
      rawurlencode($request->get_signable_parameters()),
    );
    ...
    if ($token) {
     ...
     dbg("hashing using key $key");

    $raw = implode("&", $sig);
    //$this->base_string = $raw;


    dbg("building signature on raw data url $raw using key $key");
...
class OAuthRequest {/*{{{*/
  ...
  public static function from_request($http_method=NULL, $http_url=NULL, $parameters=NULL) {/*{{{*/
    ...
    @$http_url or $http_url = $protocol."://" . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
    ...
    // Note we can't use $_GET because PHP mutilates the variable names
    ...
    // so that we can include them in the signature
      ...
      if ($http_method == "GET") {
        $req_parameters = $query_string_params;
      } 
      ...
      $req = new OAuthRequest($http_method, $http_url, $query_string_params);
    }
    else if ($http_method == "POST") {
  ...
  public function get_normalized_http_url() {/*{{{*/
    ...
    if(isset($parts['port'])) {
      $port = $parts['port'];
      if((($parts['scheme']=='https') && ($port == '443')) || ((($parts['scheme']=='http') && ($port == '443')))) {
        $port = "";
      }
      else
        $port = ':'.$port;
...
class OAuthServer {/*{{{*/
  ...
  public function fetch_access_token(&$request) {/*{{{*/
    ...
    dbg("got consumer {$consumer->key} / {$consumer->secret}");
  ...
  private function get_consumer(&$request) {/*{{{*/
    ...
    if (!$consumer_key) {
      throw new OAuthException("Invalid / blank consumer key");
    }

    dbg("lookup $consumer_key");
  ...
  private function get_token(&$request, $consumer, $token_type="access") {/*{{{*/
    ...
    dbg("token value is $token_field");
