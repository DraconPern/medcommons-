...
function formatLocalDateTime(date){
  ...
  formattedDate += twodigits(date.getHours() > 12 ? date.getHours() - 12 : date.getHours()) + ':' + twodigits(date.getMinutes());
  if(date.getHours()<12)
    formattedDate += ' AM';
  else
    formattedDate += ' PM';
 ...
 */
function execJSONRequest(url, postdata, success, errorHandler) {
  ...
  sendXMLHttpRequest(req,postdata).addCallbacks( function(req) {
    log('got result ' + req.responseText);
    try {
...
function lightbox(body) {
    ...
    if(typeof body == 'string') {
        var wrapper = DIV({'class':'lightbox'});
    ...
    var bodyDim = getElementDimensions(body);
...
var yui_loader_is_loading = false;

/**
 * Utility function for bootstrapping YUI framework
 */
function yuiLoader(components) {

  // Already loaded - let the user invoke straight away
  if(window.YUI_initialized) {
    return { insert: function(f) { f(); } };
  }

  // Function to return if YUI is not ready yet ...
  var push_listener = {
    insert: function(f) {
      yuiListeners.push(f);
    }
  ...
  if((typeof YAHOO == "undefined") || !YAHOO.util || !YAHOO.util.YUILoader) {
    if(yui_loader_is_loading) {
      log('yui still loading ...')
      return;
    }

    yui_loader_is_loading = true;

    // Load the yui loader itself
    var head = document.getElementsByTagName("head").item(0);
    var script = document.createElement("script");
    script.setAttribute("type", "text/javascript");
    log("loading yui loader ... ");
    script.setAttribute("src", "/yui/2.7.0/yuiloader/yuiloader-min.js");
    head.appendChild(script);
    window.setInterval(yuiLoader,100);
    return push_listener;
  }

  yui_loader_is_loading = false;

  if(!components)
    components = ["connection", "utilities","button","container","datasource","json"];
  ...
  loader.insert({'base':'/yui/2.7.0/'});
  return push_listener;
}
 
/**
 * Patched version of appear which removes filter after transition 
 * finished because IE text looks funy otherwise.
 */
function appearX(id) {
    appear(id,{to:1.01, duration:0.5, afterFinish: function() {if($(id).style.filter) $(id).style.filter = 'none';} });
}
