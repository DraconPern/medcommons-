 ...
 */
function get_sharing_info($accid,$nodeKey) {
  ...
  $queryAccessResult = post_url($url,"",$nodeKey);

  // Decode JSON
  $json = new Services_JSON();
  $result = $json->decode($queryAccessResult);
  if(!$result)
      throw new SystemFailure("Failed to parse JSON from queryAccess call: ".$queryAccessResult);
  ...
  if(count($shareAccounts)>0) {
    ...
    $result = $db->query("select gi.accid as group_acct_id, gi.groupinstanceid, gi.name, p.practiceid, p.practicename, u.*
