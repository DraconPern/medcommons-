...
function make_hurl ($fn,$ln,$sex,$dob,$parent_mcid, $parent_auth, $consents, $purpose, $expiry_ts = null)
{
  ...
  try {
      ...
      if($expiry_ts)
        $remoteurl .= "&expiryDateTimeSeconds=".$expiry_ts;
...
if (isset($_POST['patientname']))
{
    ...
    if (!$svc) die ("cant fetch modservices " . mysql_error());
        ...
        $expiry_ts = calculate_voucher_expiry_date_ts($v->duration);
		$expdate = calculate_voucher_expiry_date($v->duration);
		
		list($mcid,$healthurl,$auth,$secret, $access_token)= make_hurl($first,$last,'','',$accid,$auth,$v->consents,$svc->servicename,$expiry_ts);
...
echo template("voucherform.tpl.php")->set("extrasvcdetails",$extrasvcdetails)
  ->set("r2",$r2)
  ->set("free",$free)
  ->set("acctype",$acctype)
  ->set("mobile",$mobile)
