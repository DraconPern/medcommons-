...
/**
 * Enable the given user for MOD by creating service templates,
 * ensuring they have an appropriate address book,
 * and ensuring their user account has vouchers enabled.
 */
function enable_mod($user, $enable_mod) {
  dbg("setting mod enabled to $enable_mod for user {$user->accid}");
  $groupAccId = null;
  if($enable_mod) {
    ...
    // Ensure the user has services created
    if(pdo_first_row("select count(*) as cnt from modservices where accid = ?", array($user->accid))->cnt == 0) {
        make_svcs_from_templates($user->accid);
    }
...
if(!function_exists('testif_logged_in')) {
            ...
            if (isset($values['enc'])) {
              ...
              if (isset($enc_values['mcid']))
                return array(array_get($enc_values, 'mcid', ''),
                  ...
                  array_get($enc_values, 'from', ''), $_COOKIE['mc'],
...
/**
 * Return the time corresponding to the specified duration index relative
 * to the current date / time.
 * <p>
 * Duration indexes map to time offsets as follows:
 * <ul>
 *     <li>0  -  3 days
 *     <li>1  -  5 days
 *     <li>2  -  7 days
 ...
 */
function calculate_voucher_expiry_date_ts($duration) {
  $daybumps =   array (3,5,7,0,0,0);
  $monthbumps = array (0,0,0,1,3,6);
  return mktime(0, 0, 0, date("m")+$monthbumps[$duration],	date("d")+$daybumps[$duration], date("y"));
}

function calculate_voucher_expiry_date($duration) {
  return date("Y/m/d", calculate_voucher_expiry_date_ts($duration));
}

function query_oauth_keys() {
  require_once "DB.inc.php";
  $db = DB::get();
  if(isset($GLOBALS['appliance_access_token'])) {
    return array($GLOBALS['appliance_access_token'], $GLOBALS['appliance_access_secret']);
  }
  else {
    // If not explicitly configured we will try and get the MOD keys 
    // by looking for an application registered on the local appliance.
    $app = $db->first_row("select ea_key, ea_secret from external_application where ea_code = 'MOD'", array());
    if($app == null)
      throw new Exception("MOD has not been fully set up on this appliance.  Please register an application with code 'MOD' to allow MOD to operate.");

    return array($app->ea_key,$app->ea_secret);
  }
}
