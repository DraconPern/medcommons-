...
function 	logpoll ($server,$delta)
{
$now = time();	
$stmt = "INSERT into alertlog set server='$server', delta='$delta',time='$now' ";
...
while (true) // if running from command line this will repeat
{
	...
	if (!$traceflag) $top = "MedCommons Operations Notification Alerter running at $t --- "; else
	{
...
<head>
   ...
   <meta name="robots" content="noindex,nofollow">
    ...
    <title>Alert Notifier - Checks All Servers Every $refresh Seconds from $host $self</title>
 </head>
<body >
 <h2 id='title'> MedCommons Alerter Cycle Running on $host at $t </h2>
<div id='content'>
XXX;
}
echo $top;
$first = true;
$slist = array();
$stmt = "SELECT distinct server from notifiers"; // get list of servers
$result = mysql_query($stmt) or die("Can not $stmt ".mysql_error());
...
foreach ($slist as $server)
{
	...
	trace("<h4>$t >> Polling http://$server/images/greenball.png </h4>");
	$greenball = "http://$server/images/greenball.png";
	// measure time
	$t1 = microtime(true);	
	...
	else
	{
		...
		$stmt = "SELECT * from notifiers where server='$server' "; // get list of servers
		$result = mysql_query($stmt) or die("Can not $stmt ".mysql_error());
...
<li><a href=https://www.medcommons.net/iga/status >MedCommons Production Systems</a></li>\n
...
<html>
<head>
  <title>MedCommons Server Failure on $server at $t</title>
</head>
<body>
$msg
</body>
...
XXX;
					$status = mail($endpoint,"MedCommons Server Failure on $server at $t",$wrapped,$headers);
					$emailcount++;
...
XXX;
	$msg = <<<XXX
Alert on $server - see https://www.medcommons.net/iga/status and https://ci.myhealthespace.com/iga/status
XXX;
	$status = mail($endpoint,"$server failure",$msg,$headers);
	$phonemailcount++;
}
if (!$status ) trace ("<h5>Mail was not accepted for delivery</h5><p>$headers</p>");
$noticecount = $row['noticecount']+1;
$stmt = "Update notifiers set lasttime = '$now', noticecount ='$noticecount'
...
if (!$traceflag) echo " cycle $cyclenum sent $emailcount emails and $phonemailcount msgs at $t\r\n";
else  {
	echo "</body></html>"; exit;  // once only for html page
}
$timenow = time();
if ($timenow<($starttime+60)) sleep(60- ($timenow-$starttime));
} // of outer while loop
?>
