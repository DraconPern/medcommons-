 ...
 *
 * $Id: mc_oauth_client.php 8391 2010-02-08 21:20:43Z ssadedin $
 */
require_once "./OAuth.php";
require_once "./JSON.php";


class ConsumerNotRegisteredException extends Exception {
  function __construct() {
    $this->message = "The supplied consumer was not registered with the providing host";
  }
}
 ...
 * access tokens.
 *
 * The sequence of calls to first create an oauth token looks like so:
 *
 ...
 *
 *      // api->access_token->key,  api->access_token->secret should now be stored for future
 *      // reference (eg. in database)
 *
 *   4. // On callback page or later, make api calls
 *      api->get_ccr(accid) ...
 *
 * Once the token is created and stored, you can then later instantiate the ApplianceApi 
 * directly using the token to make calls on the user's behalf.
 *
 *   1.  $api = new ApplianceApi($consumerToken, $consumerSecret, $applianceUrl, $accessToken, $accessSecret);
 *
 *   2.  $api->get_ccr($accid) ...
 *
...
class ApplianceApi {
  ...
  /**
   * Share the given account's PHR to a cell phone
   *
   * @param accid       account to share (must be authorized to access with at least R permissions)
   * @param phoneNumber phone number to share with
   * @param carrier     a supported carrier - AT&T, Sprint PCS, Verizon or T-Mobile
   * @param firstName   first name of recipient
   * @param lastName    last name of recipient
   *
   * @return object with 'trackingNumber' attribute equal to tracking number created
   */
  public function share_phr_to_phone($accid, $phoneNumber, $carrier, $firstName, $lastName) {
    if(!isset($this->gateways[$accid]))
      $this->gateways[$accid] = $this->find_storage($accid);

    $result = $this->call_json($this->gateways[$accid]."/ShareByPhone.action", 
                                array("shareAccountId"=>$accid,"phoneNumber"=>$phoneNumber,"carrier"=>$carrier,
                                      "firstName"=>$firstName, "lastName"=>$lastName,
                                      "auth"=>$this->access_token->key));

    if($result->status != "ok") 
      throw new Exception("Failed to share account $accid to phone $phoneNumber: ".$result->error);
  ...
  public function get_request_token($accid) {
    ...
    // dbg("Checking match for invalid consumer ...");

    if(preg_match("/Invalid consumer/",$result) == 1) 
