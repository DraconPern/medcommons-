...
//	$emailcontact = z('EmailContact');
	...
	$redir = "?handler=adminlogin&h=$h"; // just stay in here for now

	dosql ("Insert into aProviders set role='consultant', email='$Email' , mcid='$mcid', provider='$Email', department='$department',
...
function consultant_registration_page($errdecorators=array())
{
	...
	"<a href='?handler=adminconspage&h=$h'>Consultants Directory</a> &gt; Enroll";
	$substitutions = array (
...
'_errormainclass'=>'hideerror', 
	...
	return loadpage("gconsregister.htm", "?handler=consreghandler&h=$h",
...
'$$$program$$$'=>$vn,
	...
	dosql ("Insert into aProviders set role='refphys', email='$Email' , mcid='$mcid', provider='$Email', department='unspecified',
...
'_errormainclass'=>'hideerror', 
	...
	return loadpage("grefphysregister.htm", "?handler=refhandler&h=$h",
	$defaultfields, $substitutions, $errdecorators);
...
'$$$program$$$'=>$vn,
	...
	$mcid = '9039064809625539';
	//Make a group, and make it a service
	$groupname = "$hospitalname"; // get an upload form url too
	$uploadformurl = 'http://ci.myhealthespace.com/9092031177683982/upload?accid=9092031177683982';


	//finally inset gunk into database


	dosql ("Insert into aHospitals set hospital='$hospitalname', indprogram='$programind',logourl='http://ci.myhealthespace.com/images/Logo_246x50_Transparent.gif' ");
	$hospitalind = mysql_insert_id();
	dosql ("Insert into aServices set service='$groupname',uploadformurl='$uploadformurl', indhospital='$hospitalind' ");
	$serviceind = mysql_insert_id();
	$programs =  dosql ("Select urlpath  from aPrograms where ind='$programind'");
	if (!($program = mysql_fetch_object($programs))) die ("Cant find program after inserting hospital record");

	// goto the users brand new dashbaord and set that as his startpage dashboard
	$redir = "?h=$hospitalind&s=9&d=$serviceind&v=$programind"; // just stay in here for now
...
'_errormainclass'=>'class=hide', //class=hideerror to hide error field
	...
	//https://ci.myhealthespace.com/acct/g.php?handler=adminlogin&h=71&v=3&d=60
	// goto the users brand new dashbaord and set that as his startpage dashboard
	$redir = "?handler=adminlogin&h=$hospitalind"; // just stay in here for now
...
function practice_options_page($errdecorators=array())
{
	...
	'$$$practice$$$'=>$hn,
	'$$$program$$$'=>$vn,
	'$$$crumbs$$$'=>"<a href='?handler=adminlogin&h=$h'>admin home</a> > options chooser",
	);
	$errdecorators = array (
'_errormaintext'=>"",
'_errormainclass'=>'hideerror', 
