...
function flipToFront(event)
{
    ...
    if (views && views.object && front) {
      if(getCookie('mc'))
        console.log ("Presenting Front View to User with Cookie set"); else
           console.log ("Presenting Front View to User with no Cookie");
...
function flipToSettings(event)
{
  if (notLoggedIn()) flipToFront(event); else {
    ...
    if (views && views.object && settings) {
        ...
        console.log ("Presenting Settings View to User");
           cancelOutstandingTimedRequests();
...
function flipToDebug(event)
{
  if (notLoggedIn()) flipToFront(event); else {
    ...
    if (views && views.object && info) {
        ...
        console.log ("Presenting Debug Info  View to User");
           cancelOutstandingTimedRequests();
...
function flipToPatientList(event)
{
  ...
  if (notLoggedIn()) flipToFront(event); else {
    ...
    if (views && views.object && patients) {    
        console.log ("Presenting Patient List View to User");
        cancelOutstandingTimedRequests();
...
function flipToPerf(event)
{
  ...
  if (notLoggedIn()) flipToFront(event); else {
    ...
    if (views && views.object && perf) {
        ...
        console.log ("Presenting Perf View to User");
           cancelOutstandingTimedRequests();
...
function gotoPatient (p)
{
  ...
  if (notLoggedIn()) flipToFront(event); else {
   ...
   // start the ajax request going for the activity list
    ...
    console.log ("Present Patient View for "+p.PatientIdentifier); views.object.setCurrentView(document.getElementById('onepatient'),true,true);
 }
}
...
var loadActivityList =  function ()
            ...
            jQuery.getJSON(mc_rest('ws/ijsactivitylist.php'),
                ...
                function(res) {
                if(res.status != 'ok') {                
                    console.log('An error occurred with activitylist: \n\n'+res.message);
                   loginError(res.message);
                    return true;
                    }
                    else
