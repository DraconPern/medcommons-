 ...
 // alert("got called back state "+req.readyState+" status"+ req.status);
  if (req.readyState == 4 && req.status == 200 ) 
  {
     ...
     // set the basic color scheme back
     	document.getElementById("pagecontent").style.color= 'black';
   ...
   if ( document.getElementById('serverTime'))
	...
	if ( document.getElementById('buildt')){
     ...
     $('.buildTime').sparkline(clienttimes, {type: 'bar', barColor: 'green'});
 		...
 		document.getElementById("pagecontent").style.color= 'red';
...
function ajax_machine2 (url,secs,pow2)
	{
	...
	// this is where the ajax call goes to get JSON
	  setInterval(function(){ loadJavascript( url ) }, 1000*secs); // every 10 seconds?
	 
	   loadJavascript( url ) //*Call once to get started */
	} // the stuff down below is specific to dashcode on the iphone

// everything above the line is pretty much standard

//
// this needs to be rewritten to accommodagte the common code above
//
