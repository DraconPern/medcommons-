...
if [ -n "${FILES}" ] ; then
  #
  # Dump DB -- just in case
  #
  if mysqldump --lock-tables --all-databases > /var/local/db-install_backup.sql ; then
  	echo "Backup of mysql databases (on /var/local/db-install_backup.sql) compeleted successfully"
  else
	echo "Backup of mysql databases failed - please investigate."
  fi
fi
