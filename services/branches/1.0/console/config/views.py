...
from config.models import MCProperty, parse_property, MCFeature
...
@login_required
def feature_req(request):
    errors=''
    
    features = MCFeature.objects.all()
    
    if request.POST:
        print "Saving features"
        for f in features:
            if(request.POST.has_key(f.name)):
                f.enabled = True
            else:
                f.enabled = False
            f.save()
            print "Saved feature " + f.name + " => " + str(f.enabled)
    
    for i in features:
        print i.name + ' ' + i.description + ' => ' + str(i.enabled)
        
    print "Here we are, requesting features again! - I found " 
    print features
    return render_to_response('config/features.html', 
                              default_context(request, features=features, errors=errors))
