...
cp -a ${FB_ROOT}/router/demo/build/installer/tomcat/* $DST/opt/gateway &&
cp -a ${FB_ROOT}/router/demo/build/installer/tomcat/common/classes/* $DST/opt/apache-tomcat/common/classes/ &&
pushd $DST/opt/gateway/webapps/gateway &&
mv ../gateway.war . &&
jar xf gateway.war &&
popd &&
pushd $DST/opt/gateway/native/jai &&
cp -a ${FB_ROOT}/router/demo/lib/jai/* .
popd &&
cp SOURCES/gateway $DST/etc/init.d &&
cp SOURCES/LocalBootParameters.properties SOURCES/server.xml $DST/opt/gateway/conf &&
cp ${FB_ROOT}/services/${FB_TRUNK}/config/linux/common/etc/httpd/conf.d/gateway_ajp.conf $DST/etc/httpd/conf.d &&
cp ${FB_ROOT}/services/${FB_TRUNK}/config/linux/common/etc/httpd/conf.d/router_ajp.conf $DST/etc/httpd/conf.d &&
