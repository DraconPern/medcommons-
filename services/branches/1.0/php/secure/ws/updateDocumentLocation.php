...
class updateDocumentLocationWs extends securedbrestws {
	...
	function xmlbody() {
		...
		$this->xm($this->xmnest("inputs", $this->xmfield("ekey", $ekey).$this->xmfield("node", $node).$this->xmfield("intstatus", $intstatus).$this->xmfield("guid", $guid)));
		...
		$this->xm($this->xmnest("outputs", $this->xmfield("docid", $docid).$this->xmfield("status", $status)));
	}
}
