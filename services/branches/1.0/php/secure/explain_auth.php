...
<?
    ...
    $r = mysql_query("select d.*, l.node_id from document d, document_location l where d.guid = '$guid' and l.document_id = d.id");
    if(mysql_num_rows($r) == 0) {
    ...
    while($doc = mysql_fetch_object($r)) {
      echo "<tr><td>{$doc->id}</td><td>{$doc->storage_account_id}</td><td>{$doc->node_id}</tr>";
      $document_ids[]=$doc->id;
    }
    echo "</table>";
