...
function removeApplication($user)
{
	...
	if ($r) {
		...
		// delete the entry from our team tables as giver
		$q = "delete from teams  where userfbid='$user'";// and mcid = '$mcid'";
		$result = mysql_query($q);
		if (!$result) {	logHBEvent($user,'nysql_error',$q.' '.mysql_error());  die("cant   $q ".mysql_error());}
	
		$q = "delete from teams  where teamfbid='$user'";// and mcid = '$mcid'";
		$result = mysql_query($q);
		if (!$result) {	logHBEvent($user,'nysql_error',$q.' '.mysql_error());  die("cant   $q ".mysql_error());}
