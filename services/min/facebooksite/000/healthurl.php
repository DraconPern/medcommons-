...
function healthurl_consents($u,$t){
...
function healthurl_activity($u,$t){
  ...
  try {
    ...
    $appname = $GLOBALS['healthbook_application_name'];
    $dash = hurl_dashboard($u->fbid,'activity'); // was mcid behaved wierdly
  ...
  $dash
    <fb:explanation>
      <fb:message>$appname Activities -- $my</fb:message>
      <a href='patientdetails.php' target='details'>Open full Patient Details</a>
      $activity_log
  </fb:explanation>
  </fb:fbml>
XXX;
...
function healthurl($facebook,$u, $t){
...
list($u,$t) = mustloadtarget($facebook,$user);
	switch ($op)
	{
		case 'f': {    $markup = healthurl_forms($u,$t)  ;break;}
		case 'a': {  $markup = healthurl_activity($u,$t)   ;break;}
		case 'i': {    $markup = healthurl_info($u,$t)  ;break;}
		default : {  $markup = healthurl($facebook,$u,$t)   ;break;}
	}
