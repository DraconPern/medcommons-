...
if (!($u=my_resolveEmail($email,$password))) die ("No such user /invalid password");
	...
	$row = $db->first_row("select * from users where mcid= ?",$u->mcid);
	if(!$row) 
		die('No such user / invalid password on re-read of '.$u->mcid);


		echo "gdashboard loginservice has $row->mcid";
...
//		$info = get_full_account_info($info);
		...
		if($allpatients) {
			...
			$patients = my_query_patient_list($practiceIds, $maxpatients,0,"", " AND e.ViewStatus = 'Visible' ", $customFields);
		}
