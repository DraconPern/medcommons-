...
function consultant_loggedin_page()
{
	...
	require "g4patientlist.inc.php"; // fault it in
	list($hn,$hlogo,$progind,$vn,$servicename,$serviceind) = ctx($h);

if (($h>=10)&&($h<20))
	return show_consultants_dashboard($h,'PLAYERS'); // sports groups
else
	return show_consultants_dashboard($h,'PATIENTS');
}
...
function admin_loggedin_page()
{
	...
	// add enhanced links
if (($h>=10)&&($h<20))
	return show_consultants_dashboard($h,'PLAYERS').$x; // sports groups
else
	return show_consultants_dashboard($h,'PATIENTS').$x;
}
