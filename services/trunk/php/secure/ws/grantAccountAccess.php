 ...
 * @param es_tracking_number - a token to act of parent token created for external share.
...
class grantAccountAccessWs extends securejsonrestws {
 ...
 function jsonbody(){
    ...
    $es_tracking_number = req('es_tracking_number');
    if($es_tracking_number && !is_valid_tracking_number($es_tracking_number))
      throw new Exception("$es_tracking_number is not a valid tracking number");
    ...
    if($es_identity) {
        ...
        $esId = $db->execute("insert into external_share (es_id, es_identity, es_identity_type, es_first_name, es_last_name, es_tracking_number)
                                values (NULL, ?, ?, ?, ?, ?)",
                                array($es_identity, $es_identity_type, $es_first_name, $es_last_name, $es_tracking_number));
