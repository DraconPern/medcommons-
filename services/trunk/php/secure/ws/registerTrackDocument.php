 ...
 */
class registerTrackDocumentWs extends securedbrestws {
    ...
    try {
      ...
      $node = $this->cleanhost();
      if(!$node)
        throw new Exception("Unable to resolve caller as valid node");
	  ...
	  $expiry = get_account_expiry($storageId);

      // Only create the document if it is not already there    
      $docids = $db->query("select id from document where guid = ? and storage_account_id = ?",array($guid,$storageId));
      if(count($docids) == 0) {
        // add to the document table
        $doc_id = $db->execute("INSERT INTO document (guid,storage_account_id) VALUES(?,?)", array($guid, $storageId));
        
        $db->execute("insert into document_location (id, document_id, node_id)
                      values (NULL, ?, ?)", array($doc_id, $node->node_id));
      }
      else {
        $docidRow = $docids[0];
        $doc_id = $docidRow->id;
      ...
      if($mcid !== $GLOBALS['PUBLIC_MEDCOMMONS_ID']) {
	      $rights_id = 
		      $db->execute("INSERT INTO rights (account_id,document_id,rights,expiration_time) VALUES(?,?,?,?)",
	                array($mcid, $doc_id, 'RW',$expiry)); 
      }
      ...
      if(($pin !== null) && ($pin !== "") && ($constraint != "REGISTERED_EMAIL")) {
        ...
        $rights_id = $db->execute("INSERT INTO rights (document_id,es_id, storage_account_id, rights, expiration_time) VALUES(?,?,?,?,?)",
                  array($doc_id, $es_id, $rights_storage_id, $callerRights, $expiry)); // note: 3rd last param gives tracking number access to whole acct
      }
      ...
      if(is_array($rights)) {
        ...
        foreach($rights as $rs) {
          dbg("adding right ".$rs);
          $r = explode("=",$rs);
          $db->execute("INSERT INTO rights(rights_id, document_id, account_id, rights, expiration_time) VALUES (NULL, ?,?,?,?)",
                        array($doc_id, $r[0], $r[1],$expiry));
