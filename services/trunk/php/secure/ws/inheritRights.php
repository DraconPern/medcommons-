 ...
 */
class inheritRightsWs extends securedbrestws {
      ...
      */
      ...
      if($toAccountType == "openid") {
        $esId = $db->execute("insert into external_share ( es_id, es_identity, es_identity_type )
                       values (NULL, ?, 'openid')", array($toAccount));
        $toAccount = null;
      }
      ...
      if($fromAccount != $toAccount) { // don't duplicate rights for the same user.
        foreach($rights as $right) {
          ...
          $sql = "INSERT INTO rights (account_id,document_id,es_id,storage_account_id,rights,expiration_time) ".
                 "VALUES(?,$docId, $esId, $storageId,'$right->rights', ?)"; 
          $db->execute($sql, array($toAccount, $right->expiration_time));
      ...
      $this->xm($this->xmnest("inputs",
        ...
        // return outputs
        $this->xmnest("outputs", $this->xmfield("status","ok").$this->xmfield("inheritedRights", $updateRights)));
    }
    catch(Exception $e) {
        $this->xm($this->xmnest("outputs", $this->xmfield("status","failed - ".$e->getMessage())));
    }
  }
