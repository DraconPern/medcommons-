...
class Patient {
  function Patient($id, $email, $first, $last,$currentCcr=null) {
    ...
    $this->last = $last;
    $this->ccr = $currentCcr;
    $this->consents = "RW";  // What consent the demo doctor group is going to get
  }
}
...
mysql_query("delete from external_application where ea_code = 'DEMO'");
...
foreach($patients as $p) {
    ...
    insert("INSERT INTO rights (rights_id,account_id,document_id,rights,creation_time,expiration_time,rights_time,storage_account_id) 
            VALUES (NULL,'$groupAcctId',NULL,'{$p->consents}',CURRENT_TIMESTAMP,NULL,CURRENT_TIMESTAMP,'".$p->id."')")
