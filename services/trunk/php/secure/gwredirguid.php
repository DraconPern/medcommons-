...
function try_redirect()
{
  ...
  if($dest != "") {
      ...
      $url.="g=$guid&auth=$auth".($oauth?$oauth_params:""); 
  }
  else
  if($tracking=='') // If no tracking number then get there directly by guid
      $url = "$gw/access?g=$guid&a=$accid"; 
  else // There is a tracking number - use track# interface
      $url = "$gw/tracking.jsp?tracking=$tracking&accid=$accid";
    
  // Add optional parameters in the form:
  //
  //    &t=$tracking&m=$mode&c=$context      
  if($tracking) 
      $url.="&t=".urlencode($tracking);
  if($mode) 
      $url.="&m=".urlencode($mode);
  if($context) 
      $url.="&c=".urlencode($context);
  
  // Encryption of URLs disabled for now - should be re-enabled
