 ...
 */
function dbconnect() {
  global $CENTRAL_HOST, $CENTRAL_USER, $CENTRAL_PASSWORD, $CENTRAL_DB;
	mysql_connect($CENTRAL_HOST, $CENTRAL_USER, $CENTRAL_PASSWORD) or die ("can not connect to mysql");
	$db = $CENTRAL_DB;
...
$RESOLVE_AUTH_SQL = 
        "select * from rights r,  document_location l, node n, authentication_token at, document d
         ...
         left join  users u on u.mcid = d.storage_account_id
         where d.guid = '%s' 
           and ((r.document_id = d.id) or (r.storage_account_id = d.storage_account_id))
           and r.active_status = 'Active' and (r.expiration_time is NULL or r.expiration_time > NOW())
           ...
           and at.at_token in (%s)
           and at.at_expired_date_time is NULL
...
class AccountSpec {
  ...
  function getRights() {
    ...
    if($this->esId)
      $rights = $db->query("select * from rights r where es_id = ? and active_status = 'Active' and (r.expiration_time is NULL or r.expiration_time > NOW()) ", array($this->esId));
    else
      $rights = $db->query("select * from rights r where account_id = ?  and active_status = 'Active'  and (r.expiration_time is NULL or r.expiration_time > NOW())", array($this->accId));
...
/**
 * Parse a comma separated list of auth tokens into an
 * list of comma separated quoted tokens.
 * 
 * @param $ctx
 */
function quote_tokens_from_ctx($ctx) {
    $tokens = explode(",", $ctx);
    for($i=0;$i<count($tokens);++$i) {
        if(preg_match('/^[a-z0-9]{40}$/i',$tokens[$i])!==1) 
            throw new Exception("Bad format in context token string");
        $tokens[$i] = "'$tokens[$i]'";
    }
    $tokens = join(",",$tokens);
    dbg("Generated context auth string " .$tokens);
    return $tokens;
}
 ...
 * the first account.
 */
function get_authorized_accounts($authToken, $ctx=null) {
  ...
  if(preg_match("/^[a-z0-9]{40}$/i",$authToken)===1) 
    return get_authorized_accounts("token:".$authToken,$ctx);
  ...
  if(strpos($authToken,"token:")===0) {
    ...
    $accounts = array();
    if($ctx) {
        dbg("Using context auth $ctx");
        $tokens = quote_tokens_from_ctx($ctx);
        dbg("Generated context auth string " .$tokens);
        $tokenClause="at.at_token in('$token',".$tokens.")";
    }
    else
        $tokenClause="at.at_token = '$token'";
    
    $results = $db->query("select at.at_account_id, at.at_es_id, es.es_identity, es.es_identity_type, es.es_first_name, es.es_last_name
    ...
    foreach($results as $t) {
        $accounts[]=new AccountSpec($t->at_account_id,$t->at_es_id,$t->es_identity,$t->es_identity_type,$t->es_first_name,$t->es_last_name);
    }

    return $accounts;
  }

  throw new Exception("Unknown scheme in authentication token: $authToken");
}

/**
 * Returns the date on which an account expires, or null if it
 * does not expire.
 */
function get_account_expiry($accid) {
    $db = DB::get();
    return $db->first_column("select expiration_date from users u where mcid = ?", array($accid));
}

/**
 * Return all the rights entries applicable to the given guid
 */
function resolve_guid($accid, $guid, $auth, $ctx=null) {
  ...
  // Legacy: old gateways used to pass this fixed token as a workaround
  if($auth == "Gateway")
      return $rights;
  ...
  if($auth == "") { // No auth, you can only see public stuff
    ...
    $sql = sprintf($RESOLVE_PUBLIC_SQL, $guid, ""); // find entries with any rights
  ...
  else { // We have an auth token, use that
	  ...
	  $result = $db->query("select *, 'RW' as rights, '$accid' as account_id
	               from document_location l, node n, authentication_token at, users u, document d
	               left join document_charge dc on dc.dc_document_id = d.id and dc.dc_status = 'OUTSTANDING'
	               where 
	                 d.guid = ?
	                 and l.document_id = d.id
	                 ...
	                 and u.mcid = ?
	                 and (u.expiration_date is NULL OR u.expiration_date > NOW())
	                 order by at.at_priority desc, at.at_id asc", array($guid, $accid));
	
	  if($result) {
	      dbg("found direct storage access in ".count($result)." locations");
	      foreach($result as $r) {
		      $r->es_id = null;
		      $r->rights_id = null;
		      $r->expiration_time = null;
		      $r->rights = "RW";
		      $rights[]=$r;
	      }
	      return $rights;
	  ...
	  // Not found directly as storage id - check 
	  $authClause="'$auth'";
	  if($ctx) 
        $authClause.=",".quote_tokens_from_ctx($ctx);
	  
      $sql = sprintf($RESOLVE_AUTH_SQL, $guid, "", $authClause); // find entries with any rights
  }

  dbg($sql);
  $result = $db->query($sql);
  foreach($result as $r) {
      dbg("Found rights entry {$r->rights_id}");
      $rights[]=$r;
  }
 ...
 * @param ctx         - the full security context (list of auth tokens)
 * 
 * @throws Exception - for invalid inputs, database failures
 */
function get_rights($auth, $toAccount,$ctx=array()) {
   ...
   $accounts = get_authorized_accounts($auth,$ctx);
   ...
   foreach($accounts as $a) {
     if($a->accId == $toAccount) {
       ...
       $result =
         $db->query("select r.rights from rights r 
