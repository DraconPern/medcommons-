            ...
            if($editslot == -1) { // a new record - all fine, try to add to database
                ...
                if($status) { 
                  echo template("base.tpl.php")
                         ->set("content",agreerules($base,$rate,$v->serviceemail,$AcceptMarketplaceFeeForm))
                         ->set("title","Services - MedCommons on Demand Marketplace Rules")
                         ->fetch();
                  exit; 
                } 
                else 
...
echo template("base.tpl.php")
       ->set("title", "Services - MedCommons on Demand")
       ->set("content",$t)
       ->fetch();
?>
