...
abstract class notifier extends dbextiorestws {
     ...
     * 
     * @param $t    template of email to send
     * @param $m    recipient(s)  
     * 
     * Parameters a - g are arbitrary parameters that carry different data for different 
     * messages.  Common definitions described below:
     * 
     * @param $a    tracking number of CCR for CCR notifications
     * @param $b    senders comment, if any
     */
    function xmlbody () {
        // this overrides the standard xml body to preserve
        // compatibility with the old service
        ...
        if(isset($t) && ($t!="")) {
            ...
            $subject = $c ? $c :  "MedCommons Notification for $m";
            if ($v===false) {

                if($acApplianceMode) {
                    $retval = $this->send_message($message,$mcid, $t,$m,$subject, $a,$b,$c,$d,$e,$f,$g);//call the routine to send the mail
                }
                ...
                if(isset($retval)) {
                    ...
                    $insert="INSERT INTO emailstatus (time,requesturi,sendermcid,rcvremail,template,arga,argb,argc,argd,arge,argf,argg,message,status)".
                        "VALUES(NOW(),'$ref','$mcid','$m','$t','$a','$b','.".mysql_real_escape_string($c)."','$d','$e','$f','$g','".mysql_real_escape_string($message)."','$retval')";

                    mysql_query($insert) or $this->xmlend("can not insert into table emailstatus - ".mysql_error());
                }
                else{
                    $this->xm("<request_status>".$v."</request_status>\n");
                }
