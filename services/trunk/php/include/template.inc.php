 ...
 * Field Handling
 * ==============
 * Templates offer some support to deal with validating fields,
 * showing error messages and redisplaying values when they 
 * are incorrect.
 * 
 * Inside handler PHP code, POST values are marked as "fields" by 
 * registering them using one of the functions:  verify() or post().  
 * 'post' just registers the field, while verify checks it against a regular 
 * expression and if not matching, marks it as 'invalid'.
 * 
 * Inside templates, use the field() function to inject contents into 
 * value attributes of input fields.  You can then use the error_msg()
 * function to output an error message next to the field which is only
 * rendered if the field was marked as invalid.
 * 
 * Example:
 * 
 *  verify("nameAndId", "Name and ID", "/^[0-9]{2}-[A-Za-z]{1}[0-9]{3,4}-[A-Z]{1,2}$/");
 * 
 */
class Template {
  ...
  /**
   * Debugging variable - when a nested template is displayed is set to file
   */
  var $currentFile;
  ...
  function extend($parent=null) {
      if($parent)
          ...
          Template::$current_template->parentTemplate = $this->file;
          register_shutdown_function("template_flush");
      }
  }
  ...
  function close($expectedName = false) {
      ...
      if($overridden) {
          // ("{$this->currentFile}: section $expectedName overridden");
          ob_end_clean();
          ...
          if($this->vars[$name] instanceof NestedSections) {
              ...
              // dbg("output $name: ".$this->vars[$name]);
              echo $this->vars[$name];
          }
      }
      else
      if($this->parentTemplate) {
          // dbg("{$this->currentFile}: section $expectedName not overridden");
  ...
  /**
   * Return true iff one or more errors was registered in the $errors field.
   */
...
function template_flush() {
    echo Template::$current_template->fetch();
}
 ...
 *
 * @param $x            Name of POST parameter 
 * @param $fieldName    Human readable form of field name. Defaults to post 
 *                      param, upper cased.
 * @param $pattern      Regex that the field should match.  Eg: /^[0-9]{10}$/            
 * @param $msg          Error message for display if the field does not match
 */
function verify($x, $fieldName = false, $pattern = false, $msg = false) {
   $value = post($x);  
   ...
   if($pattern) {
       if(preg_match($pattern, $value) !== 1) {
           Template::$errors->$x = $msg ? $msg : $fieldName." was not in correct format"; 
       }
   }
   return $value;
}
