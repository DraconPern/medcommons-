...
if($currentCcrGuid) { // Found current ccr?
  ...
  if($nf !== "true") {
    ...
    $share = "";
    if(req('share')) {
        redirect(gpath('Secure_Url')."/router/ccrs/$a?share=true&auth=$auth".($oauth?$oauth_params:""));
    }
    else
        redirect(gpath('Commons_Url')."/gwredirguid.php?guid=$currentCcrGuid$nopage$ctx$mode$share&nf=".urlencode($nf)."&dest=".urlencode($dest)."&auth=$auth".($oauth?$oauth_params:""));
  }
