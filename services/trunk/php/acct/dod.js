 ...
 * its remote connection (see contextManager.js).
 * <p>
...
var cdRefreshTimer;
var cdsResult;
connect(commands, 'getcddrivesComplete', function(result) {
    var json = serializeJSON(result);
    if(json == cdsResult)
         return;
    cdsResult = json;
    
    replaceChildNodes($('cddrives'));
    forEach(result.roots, function(d) {
       if(!Y.Lang.trim(d.label))
           return;
       
       if(d.label.toLowerCase().indexOf('c:')!=-1) 
           return;
       
       if(d.label.toLowerCase().indexOf('local disk')==0)  
           return;
           
       if(d.label.match("^MacBook.*HD$")) 
           return;
       
       appendChildNodes($('cddrives'), OPTION({value:d.drive}, d.label));
       Y.one('#cddrives')
        .setStyle('height',(result.roots.length+1)+'em')
        .removeClass('hidden');
    });
    if(!cdRefreshTimer) {
        connect('cddrives','onclick', function(e) {
            var drvs=$('cddrives');
            var selected = filter(function(o) {return o.selected;}, drvs.options);
            $('chooseCDButton').disabled = (selected.length != 1);
        });
        cdRefreshTimer = setInterval(refreshCDs, 5000);
    }
});

connect('chooseCDButton', 'onclick', function() {
    
    if(cdRefreshTimer) {
        clearTimeout(cdRefreshTimer);
        cdRefreshTimer = null;
    }
    ...
    sendCommand("scanfolder", { selectedLocation: selected[0].value, jsonp: 'onScanResults'});
});

function refreshCDs() {
    sendCommand("getcddrives");
}
...
function showCDInstructions() {
  if($('selectSourceStep') && $('selectSourceStep').style.display != 'none')
...
addLoadEvent(function() {
    ...
    */
    ...
    Y.Env.base = '/yui/3.2.0/';
    Y.config.base = '/yui/3.2.0/';
    Y.use('swfdetect','io-xdr','io-queue', function(Y) {
        useFlashTransport = Y.SWFDetect.isFlashVersionAtLeast(9);        
        if(useFlashTransport)  {
            enableAutoConnect = true;
            log("Detected Flash version " + Y.SWFDetect.getFlashVersion() + " : Enabling Flash");
        }
        
        versionCheckTimerId = setTimeout(function() {
            if(!webstartVersionCheck(REQUIRED_JAVA_VERSION))
                startVersionCheckApplet();
        },enableAutoConnect ? 4000: 500);
        
        if(useFlashTransport) {
            // Initialize the define transport
            Y.io.transport(DDL_FLASH_CONFIG);
            log("hooking DDL connection to xdr ready event");
            Y.on('io:xdrReady', connectDDL);
        }
        else
            connectDDL();
    });
    ...
    connect('submitProblem', 'onclick', function(evt) {
        ...
        var formData = queryString(document.supportForm);
        var env = [];
        if(callersOrderReference)
            env.push('Order Reference='+callersOrderReference);
        for(var i in Y.UA) {
            env.push(i+'='+Y.UA[i]);
        }
        
        formData += '&env=' + encodeURIComponent(env.join(","));
        execJSONRequest('/router/ProblemReport.action', formData, function(response) {
            if(response.status == "ok") {
                if(ddlRunning) {
                  execJSONRequest('/router/ProblemReport.action', 
                      {jslog:true,problemId:document.supportForm.problemId.value, description: logLines.join('\n')}, function(){});
    ...
    connect('browseDocumentFile', 'onclick', function() {
        showCDInstructions();
    });
});
...
function connectDDL() {
    ...
    log("Initiating connection to DDL");
...
function isAppletLoading() {
    ...
    // Applet broken (!) on Mac / Chrome, don't wait for it
    if(Y.UA.os == "macintosh" && Y.UA.chrome)
        return false;
    ...
    try {
        ...
        log("Applet not loaded because vcheck.isActive() failed: " + e);
        return true;
    }
...
function showInstallAdvice() {
    ...
    // Sucks, but applets are broken on the Mac for Chrome
    var isMacChrome = (Y.UA.os == "macintosh" && Y.UA.chrome);
    if(!ddlRunning) {
        try {
	        ...
	        if(webstartVersionCheck(REQUIRED_JAVA_VERSION) || isMacChrome) {
	            appearX('installDDLStep');
...
connect(commands,'scanfolderComplete',function(result) {
    ...
    execJSONRequest('/router/ddl?getResult', 'ddlid='+ddlId+'&key='+result.key+'&cmd=scanfolder', function(res) {
        ...
        initScanResults(studies, result.parsedName);
    });

});


function initScanResults(studies,patientName) {
    ...
    if(!$('selectData')) {
        ...
        if(patientName) {
            params.givenName = patientName.givenName;
            params.familyName = patientName.familyName;
            log("Patient name = " + queryString(patientName));
        }
...
/**
 * Overrideable defaults that will be sent when upload begins
 */
var uploadParams = {};

var blueButtonFileUploaded = false;
...
function startUpload(sourceLocation, dataSource, action,  params) {
    ...
    if($('document').value) {
        createBlueButtonPatient(sourceLocation, action,  params, startDDLUpload);
    }    
    else {
        execJSONRequest('create_upload_patient.php',queryString(dataSource)+'&'+queryString(params),function(result) {
            ...
            try {
                if(result.status == 'ok') {
                    startDDLUpload(sourceLocation, action,  params, result);
                }
                else {
...
function createBlueButtonPatient(sourceLocation, action,  params, callback) {
    
    log("Submitting blue button file to create patient");
    
    Y.use("io-upload-iframe", function(Y) {
        Y.io(localGatewayRootURL + '/router/NewPatientFromBlueButton.action?'+queryString({ sponsorAccountId: groupAccountId}), {
            method: 'POST',
            form: {
                id: 'blueButtonFileForm',
                upload: true
            }, 
            on: {
                start: function() { log("file uplaod start"); },
                complete: function(id, o, args) { 
                    log("fileupload complete: " + o.responseText);
                    try {
                        var obj = Y.JSON.parse(o.responseText);
                        if(obj.status != "ok") 
                            throw "Failed to import provided Blue Button File: " + obj.error;
                        log("Successfully created patient " + obj.patientMedCommonsId + " from blue button file");
                        var patientDetails = {
                            authToken: obj.auth, 
                            gwUrl: localGatewayRootURL + '/router/',
                            accid: obj.patientMedCommonsId,
                            healthUrl: localGatewayRootURL + '/' + obj.patientMedCommonsId,
                            patient: merge(obj, {accessAuth: obj.auth}) // todo: make an access auth in NewPatient service
                        };
                        callback(sourceLocation, action,  params, patientDetails);
                    }
                    catch(e) {
                        alert("A problem occured while importing your blue button file: \n\n" + e);
                    }
                },                failure: function() { log("file upload failed"); }
            }
        });
    });
    ...
    //  Submit file upload form to hidden frame
    var frm, frameName = 'upload'+(new Date()).getTime();
    appendChildNodes(document.body,
            frm = createDOM('IFRAME',{'name':frameName, src:'about:blank', style:'display: block;'}));
    
    $('blueButtonFileForm').target = frameName;
    connect(frm, 'onload', function() {
        var response = scrapeText(frames[0].document.body);
        alert('Blue button file uploaded: ' + response);
        try {
            var obj = Y.JSON.parse(response);
            if(obj.status != "ok") 
                throw "Failed to import provided Blue Button File: " + obj.error;
            // {status:'ok',patientMedCommonsId:'1259366818441568',currentCCRGuid:'2aa4462064c5750ae5e827f52b9e479b3272a623',auth:'48b3f1f2e2b7476e3c00151af0f44b4e370125c0',secret:'58925ddf4d9ebc644e2da7f391a41c120c614dad'}
            ...
            log("Constructing patient details");
            var patientDetails = {
                authToken: obj.auth, 
                gwUrl: localGatewayRootURL + '/router/',
                accid: obj.patientMedCommonsId,
            ...
            log("Passing result " + response + " to callback");
            callback(sourceLocation, dataSource, action,  params, patientDetails);
    ...
    $('blueButtonFileForm').submit();
    log('Submitted blue button file - waiting for load');
    
}


function startDDLUpload(sourceLocation, action,  params, result) {
    var defaults = { auth: result.authToken, 
        gwUrl: result.gwUrl,
        groupname: result.groupName,
        groupAccountId: result.groupAccountId,
        accountid: result.accid,
        sourceLocation: sourceLocation,
        storageid: result.accid,
        jsonp: 'uploadStarted'
    };
    var args = merge(defaults, params, uploadParams);
...
function uploadStarted(result) {
    ...
    if(result && result.status != 'ok') {
        ...
        // See dodx.tpl.php if modifying here
        if(callersOrderReference) {
            execJSONRequest('update_order_status.php', 
                    ...
                    function(response) {
                        if(response.status != 'ok') {
                            ...
                            signal(ddlEvents, 'uploadStarted');
                    }
            );
        }
        else 
...
function launchJNLP(app) {
    ...
    if(Y.UA.chrome)
        appendChildNodes('waiting', DIV({style:'margin: 2em;'}, IMG({src: 'images/ddl_jnlp_chrome_download.png'})));
...
function startVersionCheckApplet() {
  ...
  /*
  $("applet").innerHTML = 
      '<APPLET id="vcheck" style="display: block;" name="vcheck" CODE="DDLChannelApplet.class"'
      +'archive="ddlchannelapplet.jar" WIDTH="0" HEIGHT="0" MAYSCRIPT>'
      +'<param name="jnlp_href" value="ddlchannelapplet.jnlp"></APPLET>';
      */
...
var notifiedJavaVersion = null;
function notifyVersion(v) {
    log('Java version: ' + v);
    notifiedJavaVersion = v;
}
