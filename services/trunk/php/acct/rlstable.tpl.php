...
<?
  ...
  $ddl_statuses = array(
    "DDL_ORDER_ACCEPTED" => "New",
    "DDL_ORDER_MATCHED" => "Uploading",
    "DDL_ORDER_XMITING" => "Uploading",
    "DDL_ORDER_UPLOAD_COMPLETE" => "Uploaded",
    "DDL_ORDER_DOWNLOAD_COMPLETE" => "Downloaded",
    "DDL_ORDER_COMPLETE" => "Stored",
    "DDL_ORDER_ERROR" => "Error"
  );
  
  $downloadableStatuses = array("DDL_ORDER_UPLOAD_COMPLETE", "DDL_ORDER_DOWNLOAD_COMPLETE","DDL_ORDER_COMPLETE");
  ...
  foreach($rows as $l) {
    ...
    if($viewerurl=='')
      ...
      $anchor = "<a title='Open HealthURL CCR for this Patient' target='ccr' href='$href' onclick='if(setGWCookie) setGWCookie(\"$viewerurl\");' >$dateTime</a>"; 
      
    if($l->dicom_order_status) {
        $dicomOrderStatus = isset($ddl_statuses[$l->dicom_order_status]) ? $ddl_statuses[$l->dicom_order_status] : 'Unknown Status';
    }
        
?>
    <tr id='row_<?=$patientidentifier?>' class='<?=$rowclass?>' <?/*onclick='expandPatientRow("<?=$patientidentifier?>","<?=$l->gwUrl?>")' */?> >
      <td title='Open patient in Viewer (New window)' class='rlsPn leftCell'>
         <a href="<?=$href?>" onclick="return openCcrWindow('<?=$href?>',<?=$hidden ? 'true' : 'false'?>, '<?=$patientidentifier?>');" 
                              target="ccr"><img src='images/<?=$hurlImg?>'/></a>&nbsp;
         <a href="<?=$href?>" onclick="return openCcrWindow('<?=$href?>',<?=$hidden ? 'true' : 'false'?>, '<?=$patientidentifier?>');" 
                              target="ccr"><?=$patientName?></a>
      </td>
      <td class='rlsTm'><?=$dateTime?></td>
      <td class='rlsPps'><?=$purpose?></td>
      <td align='left' id='r<?=$cc?>' class='statusCell' title='Status'>
      <?if($l->dicom_order_status):?>
         <?=ent($dicomOrderStatus)?>
      <?elseif($l->couponum):?>
        <?=$l->couponstatus?>
      <?elseif($cc):?>
      ...
      <td class='iconCell'>
        ...
        <a href='javascript:toggleMember("<?=$patientidentifier?>")'><img src='images/<?=$l->pp_name?"user_dark.png":"user_outline.png"?>'/></a>
        ...
        <?elseif($l->wi_available_id || (array_search($l->dicom_order_status,$downloadableStatuses)!==false)):?>
        ...
        <?if($l->couponum):?>
          <a href='/mod/voucherprint.php?c=<?=$l->couponum?>&reprint'
             ...
             title='Voucher <?=$l->voucherid?> was issued for this patient'
            ><img src='images/voucher-icon.png' style='top: -2px;' /></a>
        <?else:?>
	        <img src='images/spacer.gif' width='16'/>
        <?endif;?>
        <?if($messages):?>
        ...
        <td class='iconCell rightCell' style='text-align: left;'>
        &nbsp;
      </td>
    </tr>
<? } ?>
  <?if(count($rows) == 0):?>
    <tr><td class='leftCell' colspan='5'><p>No Entries Available</p></td><td class='rightCell'>&nbsp;</td></tr>
  <?endif;?>
  <tr><td class='leftCell' colspan='5'>&nbsp;</td><td class='rightCell'>&nbsp;</td></tr>
  <tr id='registryTableBottomRow' messageTimestamp='<?=$messageTimestamp?>'>
    <td style='text-align: left;' class='leftCell' colspan='2'>
    ...
    <td align='right' colspan='4' id='pageLinks' class='rightCell'><?=$pageLinks?></td>
  </tr>
