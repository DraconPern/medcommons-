...
<feed xmlns="http://www.w3.org/2005/Atom" xmlns:mc="http://medcommons.net/patientdata">
  ...
  <link href="<?=xmlentities($url)?>"/>
<?if(isset($updated)):?> 
  <updated><?=$updated?></updated>
<?endif;?>
  <author> 
    <name><?=xmlentities($practicename)?></name>
  </author> 
  <id><?=xmlentities($url)?></id>
<?
  $now = time();
  $odd = false;
...
?>
  ...
  <entry>
    ...
    <link href="<?=xmlentities($href)?>"/>
    <id><?=$patientidentifier?></id>
    <updated><?=$updated?></updated>
    <summary><?=xmlentities($purpose)?></summary>
    <author>
        <uri><?=xmlentities($href)?></uri>
    </author>
<?if($l->order_status):?>
    <mc:status><?=$l->order_status?></mc:status>
    <mc:order-reference><?=xmlentities($l->order_reference)?></mc:order-reference>
    <mc:order-patient-id><?=xmlentities($l->patient_id)?></mc:order-patient-id>
    <mc:order-protocol-id><?=xmlentities($l->protocol_id)?></mc:order-protocol-id>
    <mc:guid><?=$l->Guid?></mc:guid>
    <?if($l->auth_token && $l->auth_secret):?>
    <mc:auth-token><?=$l->auth_token?></mc:auth-token>
    <mc:auth-secret><?=$l->auth_secret?></mc:auth-secret>
    <?endif;?>
<?endif;?>
  </entry>
<?endforeach;?>
