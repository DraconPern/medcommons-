...
<?/*
<div class='headertip roundcorners'>
    <h2>Need Sample Data?</h2>
    <p>MedCommons has a repository of sample images that you can use to try out our service.
    </p>
    <p>  <a href='/help/samples.html' target='sampledata'>Click Here to Get Sample Data</a></p>
    <p class='hitop'>
...
<div id='wholeDataSourceStep'>
  ...
  <div id='fillOutFormStep' class='section <?if(req("step")!="2"):?> hidden<?endif;?>'>
    ...
    <div id='cdinstructions' class='hidden'>
        ...
        <h3>1. Blue Button File (Optional) <img width=18 height=18 src='images/bluebuttonup.png'></h3>
        <div id='documents'>
            <form name='blueButtonFileForm' action='/router/NewPatientFromBlueButton.action' id='blueButtonFileForm' enctype="multipart/form-data" method='POST'>
            <p>If you have a VA or Medicare Blue Button file that you would like to include
               with the images, click the button below to locate the file to include it with 
               your upload.</p>
            <input type='file' name='blueButtonFile' id='document'>
            <?if(isset($order)):?>
            <input type='hidden' name='callersOrderReference' id='bbOrderRef' value='<?=urldecode($order->callers_order_reference,ENT_QUOTES)?>'>
            <?endif?>
            </form>
        </div>
        ...
        <ul id='cdinstructionlist'>
            <li>If you have not already, please insert the CD</li>
            <li>Ignore or close any autorun or other windows that the CD puts up</li>
            <li>After 10 seconds or so, click to select the CD to upload below</li>
            <li>After selecting the CD, click 'Choose Selected' to continue</li>
        </ul>
        
        <div id='selectcdbox'>
            <select name='cddrives' id='cddrives' multiple="multiple">
                <option>Loading ...</option>
            </select>
            <button id='chooseCDButton' disabled="disabled">Choose Selected</button>
            <button id='browseFilesButton'>Advanced ...</button>
        </div>
