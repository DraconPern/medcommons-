...
function watchForDDLEvents(gw) {
	var scr = createDOM('script',{'src': gw+'/TransferState.action?waitForEvent&'
    	+ queryString({jsonp:'onDDLEvent',auth:auth,time:(new Date().getTime())})});
...
function addMessage(m, noUpdateDisplay) {
    
    log("Adding message " + m);
    if(typeof(m) == 'string') {
        m = { category: 'INFO', msg: m };
    }
    
    if(!m.id) {
        m.src='pg';
        m.id = pageMessageCount++;
    }
...
function addRemoteMessages(msgs) {
    forEach(map(function(msg) {
      return { category: msg.tm_message_category, msg: msg.tm_message, id: msg.tm_id, src: 'tm' };
    },msgs),function(m){ addMessage(m,false);});
}
...
function displayGroupMessages() {
  ...
  forEach(reversed(groupMessages),function(m) {
      if((m.src == 'tm') && isStartedPrompt(m)) {
        messagesSince = m.id;
      }
  });
  
  var tbl = $('messageTableBody') ? $('messageTableBody') : TBODY({id:'messageTableBody'});
  forEach(groupMessages,function(m) {
      if((m.id < messagesSince) && (m.src =='tm'))
        return;
      ...
      if(isStartedPrompt(m))  {
          ...
          ddlDetected = true;
          m.msg = "Your DDL is running.  Use the 'New' menu to upload DICOM.";
      }
      ...
      var imgSrc = messageIcons[m.category]?messageIcons[m.category]:messageIcons['*'];
      appendChildNodes(tbl,TR({id:m.id+'_'+m.src},
              TD({title:'Message '+m.id,'class':'msgicon'},IMG({src:imgSrc})),TD({'class':'msgcat'},m.category), TD(null, m.msg)));
  });
  ...
  if(!$('messagesTable')) {
	  replaceChildNodes($('messages'), 
	      ...
	      DIV({id:'messageTableWrapper'},TABLE({id:'messagesTable'},tbl))
	  );
	  
	  appendChildNodes('messages', DIV({'class':'cornerDot', style: 'left: 0px;'}));
      appendChildNodes('messages', DIV({'class':'cornerDot', style: 'right: 0px;'}));	  
  }
  
  
  if(getStyle('messagesWrapper','display')=='none')
	  appear($('messagesWrapper'));
}

function isStartedPrompt(m) {
  return (m.category == 'PROMPT') && m.msg.match && m.msg.match('DDL .* Ready');
}
...
function refreshMessages() {
    ...
    execJSONRequest('groupMessages.php',authQueryString(params),function(result) {
      ...
      addRemoteMessages(result.messages);
...
function statusSelect(i) {
  if(i>=0) {
    ...
    execJSONRequest('ws/wsUpdateStatus.php', authQueryString({cc:stxt.cc,status:savedStatus,pid:practiceId}),
      function(result) {
...
function toggleMember(accid) {
	execJSONRequest('toggle_patient_member.php',authQueryString({accid:accid}), function(result) {
		if(result.status == 'ok') {
			refreshRLS();
		}
		else {
...
function expandPatientRow(patientId) {
    
    var tr = $('row_'+patientId);
    if(selectedTR) {
        removeElementClass(selectedTR,'selected');
        removeElement(detailTR);
        if(selectedTR == tr) {
            selectedTR = null;
            detailTR = null;
            return;
    ...
    var frame;
    var newTr = TR({'class':'detailsRow'}, TD({'class':'leftCell rightCell',colspan:6}, frame=createDOM('IFRAME',
            {src: '/'+patientId+'/activity', width:'100%', height: '100%', frameborder:0, style:'width:100%'})));
    
    addElementClass(tr,'selected');
    
    var trs = $('registryTable').getElementsByTagName('tr');
    for(var i=0; i<trs.length; ++i) {
        if(trs[i]==tr)
            break;
    }
    if(i<trs.length-1) {
        tr.parentNode.insertBefore(newTr, trs[i+1]);
    }
    else
        appendChildNode(tr.parentNode, newTr);
