...
function get_full_account_info($info,$extra=false) {
  try {
    ...
    $u = pdo_query("select u.email, u.first_name, u.last_name, u.acctype, u.enable_vouchers, 
      u.enable_dod, u.active_group_accid, u.tip_state,
      p.*,gi.enable_uploads, gi.logo_url
      $extraColumns
      from users u
...
function get_upload_url($g) {
    
    $ROOTUPLOAD = gpath("Secure_Url").'/acct/putils/';
    $UploadHandler = $ROOTUPLOAD.'uh.php';
    $GroupUploadURL = gpath("Secure_Url")."/".$g->accid.'/upload';
