...
function verify_local_call() {
   ...
   */
  error_log("ELB:UTI:$acClusterMode");
  if (!$acClusterMode) {
    error_log("ELB:UTI:standalone");
...
function logout($next = false) {
	...
	if (isset($_COOKIE['mcses'])) {
		...
		if (isset($_COOKIE['mc_cursor'])) {
	    setcookie('mc_cursor', False, 1, '/');
	}
		
		
// donner - reorder and allow for 
...
if(isset($_COOKIE['mcses']) && !isset($_GLOBALS['no_session_check'])) {
    $old = intval($_COOKIE['mcses']);
    $now = time();
    if($now - $old > $acSessionTimeoutSeconds) {
      logout("/acct/login.php?prompt=expiredsession");
      exit;
    }
    else
    {
