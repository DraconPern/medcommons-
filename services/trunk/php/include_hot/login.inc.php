...
function pget_validated_account_info() {
	...
	return pget_full_account_info($info);
}
 ...
 */
function pget_full_account_info($info) {
  try {
...
class User {
  ...
  public static function load($mcid) {
    ...
    $q="select u.*, gi.logo_url from users u left join groupinstances gi on gi.accid = u.active_group_accid where u.mcid = $mcid";
  ...
  public function login($url = null) {
    ...
    //  Set cookie to allow current group and patient to be remembered and accessible to JS code
    
    $user = pget_validated_account_info($this->mcid);

	$groupName = ($user->practice?$user->practice->practicename:"no group");
    
	$groupID = ($user->practice?$user->practice->providergroupid:"no group");
    
    $value = "groupName=$groupName,groupID=$groupID,patientName=SamplePatient,patientID=0";   
    $value = rawurlencode($value);
    header('Set-Cookie: mc_cursor='.$value."; path=/", false);
...
define ('FREEZEOUT_TIME',30); // in seconds
