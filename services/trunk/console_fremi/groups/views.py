...
def search_req(request):
    q = request.GET['q']
    qs = Group.objects.filter(name__icontains = q) | Group.objects.filter(accid = q)
...
def wiz_create_req(request):
    ...
    if 'search' in request.POST or 'search.x' in request.POST:
        ...
        if form.is_valid():
            mcid = normalize_mcid(form.cleaned_data['owner_mcid'])
...
def wiz_name_req(request):
    ...
    if request.POST:
        ...
        if form.is_valid():
            p = Practice.objects.get(practiceid = g.parentid)
            g.name = p.practicename = form.cleaned_data['name']
...
def edit_req(request):
	...
	if form.is_valid():
	    ...
	    group.adminUrl = form.cleaned_data['adminUrl']
	    group.memberUrl = form.cleaned_data['memberUrl']
	    group.worklist_limit = form.cleaned_data['worklist_limit']
	    group.save()
