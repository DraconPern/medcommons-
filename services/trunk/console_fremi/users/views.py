...
def password_req(request):
    ...
    if 'skey' in request.POST:
        ...
        if skey_form.is_valid():
            curr = skey.get(skey_form.cleaned_data['skey'])
            next = skey.step(curr)

            if next == decoded_skey:
                mcuser.email = skey_form.cleaned_data['email']
                mcuser.set_password(skey_form.cleaned_data['newpw'])
                ...
                properties['newpw'] = skey_form.cleaned_data['newpw']
        ...
        if pw_form.is_valid():
            mcuser.set_password(pw_form.cleaned_data['newpw'])
            mcuser.save()

            properties = get_properties()
            properties['user'] = mcuser
            properties['newpw'] = pw_form.cleaned_data['newpw']
...
#URL = 'http://yeti:9080/identity/ws/AccountCreationServiceImpl'
...
def save_user(user, form):
    ...
    pw = form.cleaned_data['pw1']
    if pw:
        user.set_password(form.cleaned_data['pw1'])

    user.enable_simtrak = form.cleaned_data['enable_simtrak']
    user.enable_dod = form.cleaned_data['enable_dod']
...
def addgroup_req(request):
    ...
    if request.POST:
        ...
        if form.is_valid():
            g = create_group(form.cleaned_data['name'],
                             normalize_mcid(form.cleaned_data['accid']),
...
def login_as_req(request):
    ...
    return render_to_response('users/login_as.html',
                              default_context(request, mcid=pretty_mcid(mcid),
                                              mcuser=user))
@login_required
def expire_req(request):
    mcid = normalize_mcid(request.POST['mcid'])
    user = User.objects.get(mcid = mcid)
    user.acctype = 'EXPIRE_IMMEDIATE'    
    user.save()
    sql_execute("DELETE FROM practice_patient where pp_accid = " + mcid)
    return HttpResponseRedirect("user?mcid=" + mcid)