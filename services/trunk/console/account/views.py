...
from account.models import LoginTracking
...
def login_req(request):
    ...
    try:
        ...
        frozen = is_frozen(username)
        if is_frozen(username):
            warn("User " + username + " frozen due to too many login attempts")
            args['frozen'] = True
        
        user = authenticate(username = username, password = password)

        if user is not None and user.is_active and not frozen:
            info("Login by "+username+" from " + request.META["REMOTE_ADDR"])
            unfreeze(username)
            login(request, user)
            return HttpResponseRedirect(next)
        else:
            warn("Failed login for "+username+" from " + request.META["REMOTE_ADDR"])
...
def is_frozen(input):
    attempts = LoginTracking.objects.filter(userinput=input)
    print "Found " + str(len(attempts)) + " for input " + input
    if len(attempts) == 0:
        return False
    
    attempt = attempts[0]
    
    elapsed_seconds = int(time.time()) - attempt.lasttime
    return attempt.failurecounter >= 3 and elapsed_seconds < FREEZE_TIME

def increment_count(input):
    all_attempts = LoginTracking.objects.filter(userinput=input)
    if len(all_attempts) > 0:
	    attempts = all_attempts[0]
	    attempts.failurecounter = attempts.failurecounter + 1 
	    attempts.save()
    else:
        attempt = LoginTracking(userinput=input, lasttime=int(time.time()),failurecounter=1)    
        attempt.save()
