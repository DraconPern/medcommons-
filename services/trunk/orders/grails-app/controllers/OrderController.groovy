...
class OrderController {
	...
	def index = { 
		...
		if(request.ctx == 'timc') {
			validator = new TIMCOrderValidator()
		}
		...
		if(params.user_email) 
    		p.senderEmail = params.user_email
		...
		try {
			...
			if(!order.callersOrderReference) {
				order.callersOrderReference = (order.groupAccountId+System.currentTimeMillis()+Math.random()).toString().encodeAsSHA1()
				log.info "Generated order reference ${order.callersOrderReference}"
			}
