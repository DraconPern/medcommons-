...
class SendOrderNotificationsJob {
	...
	def execute() {
		...
		orders.each { DicomOrderHistory h ->
			try {
				...
				/*
				 if(h.dicomOrder.ddlStatus == OrderState.DDL_ORDER_DOWNLOAD_COMPLETE && h.dicomOrder.notificationEmail) { 
				...
				if(h.ddlStatus == OrderState.DDL_ORDER_UPLOAD_COMPLETE && h.dicomOrder.uploadNotificationEmail) { 
					log.info "Need to send notification for order " + h.dicomOrder + " / ${h.dicomOrder.callersOrderReference}" 
					notifyOrderStatus(h.dicomOrder,"Image Upload Ordered", h.dicomOrder.uploadNotificationEmail)
				}
				
				sendCallbackNotification(h.dicomOrder)
				
				if(settings.acNotifyDDLTimeout && h.ddlStatus == OrderState.DDL_ORDER_TIMEOUT_WARNING) {
					log.info "Sending timeout warning for order $h.dicomOrder.callersOrderReference" 
					sendTimeoutWarningNotification(h.dicomOrder) 
				}  
				
				if(OrderState.UPLOAD_FINAL.contains(h.ddlStatus)) {
					sendFinalNotification(h.dicomOrder)
				}
			} 
			catch(Exception e) {
	...
	def notifyOrderStatus(DicomOrder order, String subject, String emailAddress) {
		...
		sendNotificationEmail(order, [emailAddress], emailSubject, "orderNotificationEmail", binding)
	}
	
	def sendTimeoutWarningNotification(DicomOrder order) { 
		def to = settings.acNotifyDDLTimeout?:settings.acFromEmail
		sendNotificationEmail(order, [to],  "WARNING: Order $order.callersOrderReference Exceeded Timeout Threshold",
		"orderDelayNotificationEmail",
		[order: order, settings: settings ] )
	}
	
	def sendFinalNotification(DicomOrder order) {
		log.info "Order " + order.id + " reached final state.  Sending notifications"
		def emailSubject = 
						"$settings.acCommonName - Order Imaging Transfer Complete:  " + 
						order.ddlStatus.toString().replaceAll("DDL_ORDER_","")
		def to = [order.notificationEmail, order.senderEmail].findAll{it?it.trim():false}
		if(order.group?.uploadNotification) 
			to << order.group.uploadNotification
		
		if(to) {
			sendNotificationEmail(order, to,  
							emailSubject,
							"orderCompleteNotificationEmail",
							[order: order, settings: settings ] )
		}
		else 
			log.info "No notification email addresses: email not sent"
	}
	 ...
	 * 
	 * @param to            a list of recipients in string form.  Each recipient itself
	 *                      is split on commas to break into multiple possible addresses
	 * @param subject
	 ...
	 */
	def sendNotificationEmail(DicomOrder order, List to, String subject, String template, Map model) {
		def recipients = to.inject([]) { tos, r ->
			r.split(",").each {  tos << it.trim() }
			return tos
		}.unique()
		...
		try {		
			...
			// Determine the context from the group (MAJOR HACK)
			def ctx = 'orders'
			if(settings.acDefaultDODXProvider == order.groupAccountId) {
				ctx = 'timc'
			}
			model.ctx = ctx
			model.group = order.group
			model.config = ConfigurationHolder.config
			...
			recipients.each {
				DicomOrderNotification n =
					new DicomOrderNotification(order: order, recipient: '', subject: subject, status: "Unsent")
				
				try {
					MimeMessage msg = mailSender.createMimeMessage();
					msg.addRecipient(RecipientType.TO, new InternetAddress(it))
					...
					n.status = "failed"
					n.error = e.toString()	
					log.warn("Failed to send to address $it: ", e)
				}
				finally {
					n.save()
				}
