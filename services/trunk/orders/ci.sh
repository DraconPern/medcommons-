...
(
  ...
  (
    ...
    rm -f target/orders*.war

    grails -Dgrails.env=$ORDERS_ENVIRONMENT war || err "Unable to build orders war file"

    mv target/orders*.war /tmp/$ORDERS_WAR || err "Unable to move war file"
    ...
    msg "Publishing templates ..."
    curl -d "" http://localhost/cgi-bin/publish > publish.log
