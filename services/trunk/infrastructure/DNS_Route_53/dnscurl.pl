...
# Dbg
if ($debug) {
	print STDERR "[clock check:aws_key_id >>=>> " . $aws_key_id . " <<=<<]\n";
	print STDERR "[clock check:aws_secret_key >>=>> " . $aws_secret_key . " <<=<<]\n";
	print STDERR "[clock check:base_url >>=>> " . $base_url . " <<=<<]\n";
}
#####
...
# Dbg
if ($debug) {
	print STDERR "[clock check:server_date >>=>> " . $server_date . " <<=<<]\n";
}
#####
...
my ($curl_args_file, $curl_args_file_name) = tempfile(UNLINK => 0);
...
# Dbg
if ($debug) {
	print STDERR "[hmac >>=>> " . $hmac . " <<=<<]\n";
	print STDERR "[signature >>=>> " . $signature . " <<=<<]\n";
	print STDERR "[base_url >>=>> " . $base_url . " <<=<<]\n";
	print STDERR "[curl_args_file_name >>=>> " . $curl_args_file_name . " <<=<<]\n";
	print STDERR "[command_line >>=>> " . "@ARGV" . " <<=<<]\n";
}
#####

# fork/exec curl, forwarding the user's command line arguments
system($CURL, "-v", @ARGV, "--config", $curl_args_file_name);
my $curl_result = $?;
...
sub fetch_server_date {
	...
	#my $curl_output_lines = run_cmd_read($CURL, "-I", "--insecure", $url);
	my $curl_output_lines = run_cmd_read($CURL, "--progress-bar", "-I", "--url", $url, "--insecure");
