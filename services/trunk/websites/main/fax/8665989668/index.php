...
$appliance = 'http://portal.medcommons.net/';
$faxnumber = '8665989668';
...
if (!isset($_POST['xml'])) {
  error_log("fax.$faxnumber.error.post: Error: requires 'xml' post variable");
...
if (!$dom) {
  error_log("fax.$faxnumber.error.xml: Error: cannot parse XML");
  echo "Post Successful";
  exit;
}
...
try {
  ...
  error_log("fax.$faxnumber.error.barcode: Error: no barcode");
  echo "Post Successful";
  exit;
}
...
$url = gateway_url($mcid, $appliance);
$url = combine_urls($url, 'router/DataOnCallServlet');

error_log("fax.$faxnumber for $mcid posted to $url");
