...
$mcid = login_required('settingsgr.php');
$t = template('settingsgr.tpl.php');
...
if($user->practice) {
    $t->set("apiKeys", query_group_api_keys($user->practice->accid));
}

// OpenID providers have a format field.  We now put other providers in the table too
// but they should not be shown here, so filter them out.
$t->set('idps',pdo_query('select * from identity_providers where format is not NULL'));
