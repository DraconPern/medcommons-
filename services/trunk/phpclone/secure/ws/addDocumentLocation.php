...
class addDocumentLocationWs extends securedbrestws {
    ...
    $nodeId = $node->node_id;
		...
		// echo inputs
		$this->xm($this->xmnest("inputs", $this->xmfield("ekey", $ekey).$this->xmfield("node", $nodeId).$this->xmfield("intstatus", $intstatus).$this->xmfield("guid", $guid)));
		...
		if ($docid == "") {
			...
			$this->xm($this->xmnest("outputs", $this->xmfield("docid", $docid).$this->xmfield("status", $intstatus)));
		}
	}
}
