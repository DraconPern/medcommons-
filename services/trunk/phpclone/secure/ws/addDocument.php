 ...
 */
class addDocumentWs extends securedbrestws {
    ...
    try {
      ...
      $db->execute("replace into document_location (id, document_id, node_id)
                    values (NULL, ?, ?)", array($docId, $this->nodeid));
      
      $this->xm($this->xmnest("outputs",$this->xmfield("status","ok"))); 
    }
    catch(Exception $e) {
      error_log("addDocumentWs failed: ".$e->getMessage());
      $this->xm($this->xmnest("outputs",$this->xmfield("status","failed")));
