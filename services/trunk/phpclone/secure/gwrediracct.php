...
function try_redirect()
{
  ...
  //  This grabs the node(s) where the specified accid has documents. 
	...
	$query = "SELECT * FROM node WHERE node.node_id= ( select distinct document_location.node_id from document, document_location where document.id=document_location.document_id and document.storage_account_id =$accid)";
	//echo "<pre>$query</pre>";
