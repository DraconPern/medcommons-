...
<script type="text/javascript" src="DataKit/fieldedit.js"></script>
...
function onetab_vertical ($rows, $ignored, $tabkey, $tablename,$tablabel,$markup, $sortkey,$extra)
{
	...
	// this function called by generated code
	global $labels_,$fields_,$viewer_,$simtrakid_,$playerind_,$frontab_,$panebreak_,$sizes_;
	$jstop ='';
...
XXX;
	...
	// generate full page
	//
	$allplayers_=playerchoices();  // get all players
	$time = strftime('%D');
	...
	else	{$layout_ = $_GET['layout']; $backlislog= $admin."accid=$mcid_";}
	...
	$imageurl = $r->imageurl;
	$teamind = $r->teamind;
	$league = leaguenamefromteamind($teamind);
	$tk = substr($frontab_,4); //strip tab_
	$result = dosql("Select ddtable  from viewerorder where tabkey='$tk' ");
	...
	$pagetop = page_top($standalone,$playerind_, $r->name,$r->team,$league,$hurlink,'', $layout_,$simtrakid_,$imageurl);
...
if ($mcid_==0) { 
	...
	die('require_once "admin.inc.php"; envision_admin_page()'); // this is url processing gets done for the variou7s postbacks
}
// otherwise, the fast_path
else viewer_plugin($admin); // mcid is hidden variable
?>
