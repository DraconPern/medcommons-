...
$ob="<h4>Last $count rows from MedCommons Simtrak Log, most recent on top</h4><table>";
while ($r=mysql_fetch_object($result))
{
	$pt = strftime('%D %T',$r->time);
	$url = substr($r->url,0,100);
	if (strlen($url) == 100) $url .='...';
	$ob .= "<tr><td>$pt</td><td>$r->type</td><td>$r->ip</td><td>$r->grid</td><td  >$r->id</td><td>$url</td></tr>";
}
$ob.="</table>";
echo $ob;
