...
function addcontent($me,$user, $u,$t){
	$appname = $GLOBALS['healthbook_application_name'];
...
if (isset ($_REQUEST['o'])) $op = $_REQUEST['o']; else $op='';
	switch ($op)
	{
		case 'c': {    $markup = addccr($user,$t->familyfbid,$t->mcid)  ;break;}
		case 't': {    $markup = addcontent($user,$t->familyfbid,$t)  ;break;}
		case 'd': {  $markup = adddicom($user,$t->familyfbid,$t->mcid)   ;break;}
		case 'f': {    $markup = addfax($user,$t->familyfbid,$t->mcid)  ;break;}
		case 'i': {    $markup = addimage($user,$t->familyfbid,$t->mcid)  ;break;}
		case 'p': {    $markup = addpdf($user,$t->familyfbid,$t->mcid)  ;break;}
		default : {  $markup =acoverview($user,$t->familyfbid,$u,$t) ;break;}
