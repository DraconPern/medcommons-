...
if ($username=="") xmlend("username must be non-null");
	...
	mysql_select_db($db) or die ("can not connect to database $db");
 	...
 	$result = mysql_query ($query) or xmlend("can not query table User - ".mysql_error());
 	$count=0;
	...
	while ($l = mysql_fetch_array($result,MYSQL_ASSOC)) {
		...
		xm("<entry >");
	  		xm("<email>".$l['email_address']."</email>"); // preserve compatibility
	  		xm("<mcid>".$l['medcommons_user_id']."</mcid>"); // preserve compatibility

	    z($l,'hpass');z($l,'name');
		 z($l,'gateway1'); z($l,'gateway2');
		
		z($l,'serial'); z($l,'identity_provider');
//		z($l,'cert_url');
//		z($l,'cert_checked');
		z($l,'status');
