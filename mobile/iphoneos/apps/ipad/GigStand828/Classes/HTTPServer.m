...
- (BOOL)start:(NSError **)error
{
	...
	if(success)
	{
		...
		[GigStandAppDelegate sharedInstance].myLocalPort = port;  // only place where set
//		NSLog (@"re-started HTTP server  ++++++++++++++++++++++++++ running on  %@:%d +++++++++++++++++++",
...
//			   [GigStandAppDelegate sharedInstance].myLocalPort);
		...
		//NSLog(@"re-started HTTP server on port %hu", port);
