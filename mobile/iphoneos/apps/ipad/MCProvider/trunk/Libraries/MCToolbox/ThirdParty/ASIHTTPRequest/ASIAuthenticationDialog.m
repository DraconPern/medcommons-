...
- (void)orientationChanged:(NSNotification *)notification
{
    ...
    CGAffineTransform newTransform = CGAffineTransformMakeRotation(angle * (float) M_PI / 180.0f);
    ...
    // Fix the view origin
    tmpFrame.origin = f.origin;
    tmpFrame.size = self.view.frame.size;
    self.view.frame = tmpFrame;
