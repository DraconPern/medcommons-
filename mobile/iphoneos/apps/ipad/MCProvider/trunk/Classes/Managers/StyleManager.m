...
#define BACKGROUND_COLOR_DARK_KEY          @"backgroundColorDark"
...
{
    UIColor *color = [self.styleDict objectForKey: BACKGROUND_COLOR_DARK_KEY];

    if (!color)
    {
        color = [UIColor colorWithWhite: 0.125f
                                  alpha: 1.0f];

        [self.styleDict setObject: color
                           forKey: BACKGROUND_COLOR_DARK_KEY];
    }
...
- (UIImage *) defaultCCRThumbImage
{
    ...
    if (!image)
    {
       // image = [UIImage imageNamed: @"FallbackThumb-136x136.png"];
        ...
        image = [UIImage imageNamed: @"blank-136x136.png"];
