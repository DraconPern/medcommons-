...
{
    NSMutableDictionary *prefs = ((PersonStore *)[[DataManager sharedInstance] ffPatientStore]).prefs;
...
-(void) customMainPortaitControllerStoreTextData:(UITextField *)textField
{
    ...
    [[DataManager sharedInstance].ffPatientStore writePersonStore];// checkpoint
}
...
-(void) customMainViewConfirmUpload:    (UIAlertView *) baseAlert  delegate:(id)delegate
{
    ...
    if (([prefs objectForKey:@"firstname"]==@"") && (@""==[prefs objectForKey:@"lastname"]) )
    {
        ...
        if ((![[[DataManager sharedInstance] ffPatientStore] hasSubjectPhoto] ) &&([[[DataManager sharedInstance] ffPatientStore] countPartPics]==0))
        {
        ...
        else
        {
            if ([prefs objectForKey:@"dob"]==@"")
                ...
                if (![[[DataManager sharedInstance] ffPatientStore] hasPhotoAtIndex:0])
