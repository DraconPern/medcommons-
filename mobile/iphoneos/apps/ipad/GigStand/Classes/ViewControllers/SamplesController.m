...
-(UIView *) buildUI
{
	...
	self.navigationItem.titleView = [[DataManager allocTitleView:@"choose samples then  push -->" ]autorelease];	
          ...
          cellForRowAtIndexPath: (NSIndexPath *) idxPath
{
	...
	if (row < [ self->listItems count])
	{
		...
		cell.imageView.image = [DataManager allocSquareThumbRS:image size:[DataManager standardThumbSize]] ;
