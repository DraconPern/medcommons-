...
- (CFHTTPMessageRef)allocPrepareUniRangeResponse:(UInt64)contentLength;
- (CFHTTPMessageRef)allocPrepareMultiRangeResponse:(UInt64)contentLength;
@end
...
//	}
	...
	else
	{
		if([ranges count] == 1)
		{
			response = [self allocPrepareUniRangeResponse:contentLength];
		}
		else
		{
			response = [self allocPrepareMultiRangeResponse:contentLength];
		}
	}
	...
	if(tag == HTTP_REQUEST_HEADER)
	{
		...
		else
		{
			...
			if(expectsUpload)
			{
				...
				if(![NSNumber parseString:(NSString *)contentLength intoUInt64:&requestContentLength]) // was UInt64
				{
			...
			if(expectsUpload)
			{
				...
				NSUInteger bytesToRead = requestContentLength < POST_CHUNKSIZE ? (NSUInteger) requestContentLength : POST_CHUNKSIZE;
...
- (void)onSocket:(AsyncSocket *)sock didWriteDataWithTag:(long)tag
{
	...
	else if(tag == HTTP_PARTIAL_RANGE_RESPONSE_BODY)
	{
		...
		if(bytesLeft > 0)
		{
			NSUInteger bytesToRead = bytesLeft < READ_CHUNKSIZE ? (NSUInteger) bytesLeft : READ_CHUNKSIZE;
