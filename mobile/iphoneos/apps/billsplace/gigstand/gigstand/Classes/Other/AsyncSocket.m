...
// wld - hacked in march 2011 because new compiler was complaining about some unnecessary tests
...
- (CFSocketRef) allocCreateAcceptSocketForAddress:(NSData *)addr error:(NSError **)errPtr;
...
@interface AsyncReadPacket : NSObject
{
  @public
	...
	NSUInteger bytesDone; // was CFIndex
	NSTimeInterval timeout;
	NSUInteger maxLength; // was CFIndex must check for -1
	long tag;
	NSData *term;
...
- (unsigned)readLengthForTerm
{
	...
	NSUInteger i = MAX(0, (CFIndex)(bytesDone - [term length] + 1));
	NSUInteger j = MIN([term length] - 1, bytesDone);
...
- (CFIndex)searchForTermAfterPreBuffering:(CFIndex)numBytes
{
	...
	NSUInteger i = MAX(0, (CFIndex)(bytesDone - numBytes - [term length] + 1));
...
- (BOOL)acceptOnAddress:(NSString *)hostaddr port:(UInt16)port error:(NSError **)errPtr
{
	...
	if (address)
	{
		theSocket = [self allocCreateAcceptSocketForAddress:address error:errPtr];
		if (theSocket == NULL) goto Failed;
	}
	
	if (address6)
	{
		theSocket6 = [self allocCreateAcceptSocketForAddress:address6 error:errPtr];
...
- (BOOL)attachStreamsToRunLoop:(NSRunLoop *)runLoop error:(NSError **)errPtr
{
	...
	for(NSUInteger i = 0; i < [theRunLoopModes count]; i++)
	{
...
- (BOOL)setSocketFromStreamsAndReturnError:(NSError **)errPtr
{
	...
	{
		if (errPtr) *errPtr = [self getSocketError];
		//CFRelease(socket);// added by donner
		return NO;
	}
...
- (void)readDataToData:(NSData *)data withTimeout:(NSTimeInterval)timeout maxLength:(CFIndex)lengthx tag:(long)tag
{
	// there seems no case where length < - 
	NSUInteger length = (NSUInteger) lengthx;
...
**/
- (CFIndex)readIntoBuffer:(UInt8 *)buffer maxLength:(NSUInteger)length
{
