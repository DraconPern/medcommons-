	...
	if (idxPath.row <[self->thisController.searchResults count])
	{
        ...
        {	
            UIViewController *otv = [DataManager makeOneTuneViewController:
                                     [DataManager deriveLongPath:ii.filePath forArchive:ii.archive]	
                                                                     title:[tn title] 
                                                                     items:self->thisController.allTitleNames] ;
            
            UINavigationController *nav = [[[UINavigationController alloc] initWithRootViewController: otv] autorelease];
            
            [self->thisController  presentModalViewController:nav animated: YES];
    
            
			
           // [self->thisController.navigationController pushViewController:animated: YES];
          ...
          cellForRowAtIndexPath: (NSIndexPath *) idxPath
{
	...
	if (section < [ self->thisController.tunesByLetter count])
	{
		...
		if (row <[tunes count])
		{
            ...
            float fontsize  = (UI_USER_INTERFACE_IDIOM() != UIUserInterfaceIdiomPad)?18.0f:28.0f; // ipad allows fat fingers
            
			cell.textLabel.font = [UIFont boldSystemFontOfSize:fontsize]; //Change this value to adjust size
...
didSelectRowAtIndexPath: (NSIndexPath *) idxPath
{
	...
	if (idxPath.row <[tunes count])
	{
            ...
            
        {
            ...
            UIViewController   *otv  =  [DataManager makeOneTuneViewController:
                                          [DataManager deriveLongPath:ii.filePath forArchive:ii.archive] title:[tn title] 
                                                                          items:self->thisController.allTitleNames] ;
            
            UINavigationController *nav = [[[UINavigationController alloc] initWithRootViewController: otv] autorelease];
            
            [self->thisController.parentViewController   presentModalViewController:nav animated: YES];
...
@synthesize tunesByLetter,overlayView,searchResults,allTitleNames;
...
- (UIView *) allocBuildSearchUI
{
    ...
    // do not release self->searchresultstableview as it is added as subview to resultsoverlayview
	...
	outer.backgroundColor = [DataManager applicationColor];

  
	
	float fudge =0;//[DataManager topFudgeFactor];
    pframe.origin.y+=fudge;
    pframe.size.height-=fudge;
	
     // NSLog (@"outer is at %f",pframe.origin.y);
    
	
	UISearchBar *searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(pframe.origin.x, pframe.origin.y+[DataManager navBarHeight], 
                                                                           pframe.size.width, [DataManager searchBarHeight])];  // do not autorelease
	...
	//pframe.size.height -= (0.0f+100.0f); // very confused, played with this until z and then # showed up
	pframe.origin.y =[DataManager searchBarHeight]+[DataManager navBarHeight];
	pframe.size.height = outer.frame.size.height-pframe.origin.y;
   ...
   // NSLog (@"Adding mainTableView origin %f height is %f",mainTableView.frame.origin.y,mainTableView.frame.size.height);
	...
	CGRect oframe =  CGRectMake(0,[DataManager searchBarHeight]+[DataManager navBarHeight],
                                self.parentViewController.view.bounds.size.width,
                                self.parentViewController.view.bounds.size.height
								-[DataManager searchBarHeight]-[DataManager navBarHeight]);
	self->overlayView = [[UIView alloc]initWithFrame:oframe];
	self->overlayView.backgroundColor=[DataManager applicationColor];//[UIColor greenColor];
   ...
   // NSLog (@"overlayView origin %f height is %f",overlayView.frame.origin.y,overlayView.frame.size.height);
	...
	oframe = CGRectMake(0,0,
                        self.parentViewController.view.bounds.size.width,
                        self.parentViewController.view.bounds.size.height-[DataManager navBarHeight] -[DataManager searchBarHeight]);
   ...
   // NSLog (@"Adding SearchResultsTableView origin %f height is %f",oView.frame.origin.y,oView.frame.size.height);
	...
	self.navigationItem.titleView = [DataManager makeTitleView:navTitle] ;
...
- (void) dealloc
{
	...
	NSLog (@"STV dealloc *** must check carefully ");
