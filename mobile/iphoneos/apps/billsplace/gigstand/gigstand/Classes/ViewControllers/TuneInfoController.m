...
- (void) loadView
{
	...
	self.navigationItem.titleView = [DataManager makeTitleView:self->tuneTitle ];	
          ...
          cellForRowAtIndexPath: (NSIndexPath *) idxPath
{
	...
	if (row < [ self->listItems count])
	{
		...
		NSString *longpath = [DataManager deriveLongPath:ii.filePath forArchive:ii.archive];
		...
		cell.imageView.image =[ArchivesManager makeArchiveThumbnail: ii.archive ];
